...
public class BMFont
{
    ...
    static void Font()
    {
        Material mtr = Resources.Load<Material>("Fonts/MyFont_orange"); //把我们创建的材质球加载进来
        Texture2D texture = Resources.Load<Texture2D>("Fonts/MyFont_orange_0"); //把我们在位图制作工具生成的图片加载进来
        mtr.SetTexture("_MainTex", texture);//把图片赋给材质球

        Font font = Resources.Load<Font>("Fonts/MyFont_orange"); //把我们创建的字体加载进来
        XmlDocument xml = new XmlDocument();
        xml.Load(Application.dataPath + "/Resources/Fonts/MyFont_orange.fnt");//这是在BMFont里得到的那个.fnt文件,因为是xml文件，所以我们就用xml来解析
