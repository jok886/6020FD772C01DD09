...
namespace MersenneTwister
{
    ...
    public class MT19937
    {
        ...
        static private ulong []_count = new ulong[8];

        static public ulong TotalCount { get; set; }

        //static unsigned long mt[N]; /* the array for the state vector  */
        static private ulong[] mt = new ulong[N];

        // static int mti=N+1; /* mti==N+1 means mt[N] is not initialized */
        static private uint mti = N + 1; /* mti==N+1 means mt[N] is not initialized */

        /* initializing the array with a NONZERO seed */
        static public void Seed(ulong seed)
        {
            ...
            TotalCount = 0;
            Debug.Log("MTRandom seed reset!!");
        }

        /* initialize by an array with array-length */
        /* init_key is the array for initializing keys */
        /* key_length is its length */
        static public void Seed(ulong[] init_key)
        {
            ulong i, j, k;
...
/* for integer generation */
        static public ulong UInt64(bool isLog = false,int logLevel = 0)
        {
            ulong y;
            ...
            if (isLog)
            {
                //Debug.Log("rand count is:" + _count[logLevel] + ", num is:" + y + ",TotalCount:" + TotalCount + ", Frame Num:" + ControlManager.GetInstance().GameFrameNum);
                //Debug.LogFormat("rand count is :{0}, num is {1} logLevel is {2} total count is {3}, Frame Num:{4}", _count[logLevel], y,
                //    logLevel, TotalCount, ControlManager.GetInstance().GameFrameNum);
            }
