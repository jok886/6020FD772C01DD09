...
#define SUB_GR_MATCH_SIGNUP_CHECK	412									//mChen, 报名Check
...
{	
	...
	SCORE							lPlayAvgCost[4];						//AA支付消耗点数

	//mChen add,金币场模拟比赛场
	BYTE							cbMatchPlayCout;
	// 时间配置   
	SYSTEMTIME                        MatchStartTime;                //游戏开始日期
	SYSTEMTIME                        MatchEndTime;				     //游戏结束日期
};
...
{	
	...
	void StreamValue(datastream& kData,bool bSend)
	{
		...
		Stream_VALUE(lBaseScore);

		Stream_VALUE(PlayerCount);
...
{	
	...
	void StreamValue(datastream& kData,bool bSend)
	{
		Stream_VALUE(cbEndReason);

		//mChen add, for HideSeek
		Stream_VALUE(cbMapIndex);
		Stream_VALUE(wRandseed);
		Stream_VALUE(wRandseedForRandomGameObject);
		Stream_VALUE(wRandseedForInventory);
		for (int i = 0; i < MAX_INVENTORY_NUM; i++)
		{
			Stream_VALUE(sInventoryList);
		}
...
//mChen add, for HideSeek
#define SUB_GF_CREATER_PRESS_START  4									//房主是否点击开始按钮
#define SUB_GF_INVENTORY_CREATE		5									//道具生成同步：服务器通知客户端生成道具

//聊天命令
#define SUB_GF_USER_CHAT_INDEX			8									//用户聊天 mChen add
#define SUB_GF_USER_EXPRESSION_INDEX	9									//会话表情 mChen add
...
//WQ 执行奖励
#define SUB_C_AWARD_DONE         1002              //执行奖励
#define SUB_S_AWARD_RESULT       1003              //执行奖励结果

//WQ 道具消费
#define SUB_C_CONSUMPTION_INVENTORY             1004                              //道具消费
#define SUB_S_CONSUMPTION_INVENTORY_RESULT      1005                              //道具消费返回命令
#define SUB_S_CONSUMPTION_INVENTORY_EVENT       1006
//////////////////////////////////////////////////////////////////////////////////
//WQ
//奖励执行
...
{
	DWORD							dwUserID;							//用户标识
	DWORD							dwAwardGold;						//奖励金额
	BYTE                            cbCostType;                         //奖励类型
...
//道具消费信息   WQ
struct  CMD_C_InventoryConsumptionInfo
{
	DWORD               dwUserID;                 //用户标识
	BYTE                cbItemID;                  //道具ID
	WORD                wAmount;                  //道具金额
	BYTE                cbCostType;               //道具金额类型
};
struct CMD_S_InventoryConsumptionInfoResult    //WQ
{
	DWORD                   dwUserID;                 //用户标识        
	BYTE                    cbSuccess;                     //购买成功
	BYTE                    cbItemID;                       //商品ID
	DWORD                   dwFinalScore;                   //当前钻石/金币
	BYTE                    cbCostType;                    //道具金额类型
	TCHAR                   szDescribeString[256];		   //描述消息
};
struct	CMD_S_InventoryConsumptionEvent
{
	BYTE              cbChairID;           //使用道具者座位ID
	BYTE              cbItemID;            //道具ID
	DWORD             dwModelIndex;        //使用变身道具后的modelindex
	BYTE              cbStealStatus;       //隐身状态
	BYTE              cbStealTimeLeft;     //隐身剩余时间
};
...
//////////////////////////////////////////////////////////////////////////
////用户聊天
//struct CMD_GF_C_UserChatIdx	
//{
//	WORD							wItemIndex;							//文字索引
//	WORD							wSendUserID;						//发送用户
//};
////用户表情
//struct CMD_GF_C_UserExpressionIdx
//{
//	WORD							wItemIndex;							//表情索引
//	WORD							wSendUserID;						//发送用户
//};
...
#define DTP_GR_HEAD_HTTP			13									//微信头像http地址
#define DTP_GR_LAST_LOGON_IP		14									//最后登录IP地址
