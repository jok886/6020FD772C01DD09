...
//插入数据
bool CWHDataQueue::InsertData(WORD wIdentifier, VOID * pBuffer, DWORD wDataSize)//mChen edit WORD, fix size bug
{
	//变量定义
...
//插入数据
bool CWHDataQueue::InsertData(WORD wIdentifier, tagDataBuffer DataBuffer[], DWORD wDataCount)//mChen edit WORD, fix size bug
{
	//变量定义
	tagDataHead DataHead;
	ZeroMemory(&DataHead,sizeof(DataHead));
...
//获取数据
bool CWHDataQueue::DistillData(tagDataHead & DataHead, VOID * pBuffer, DWORD wBufferSize)//mChen edit WORD, fix size bug
{
	//效验变量
	...
	//获取大小
	DWORD wPacketSize=sizeof(DataHead)+pDataHead->wDataSize;
