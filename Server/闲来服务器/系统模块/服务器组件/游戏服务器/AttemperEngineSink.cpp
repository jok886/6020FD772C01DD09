		...
		case IDI_SEND_SYSTEM_MESSAGE: //系统消息
			{
				...
				while(pos != NULL)
				{
					...
					if(pTagSystemMessage->dwLastTime+pTagSystemMessage->SystemMessage.dwTimeRate < dwCurrTime)
					{
						...
						//发送消息
						DWORD wSendSize = sizeof(SendMessage)-sizeof(SendMessage.szSystemMessage)+CountStringBuffer(SendMessage.szSystemMessage);
...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, DWORD wDataSize)
{
	...
	case DBR_GR_AWARD_DONE:      //奖励处理结果返回
	{
		return OnDBPCUserAwardResult(dwContextID, pData, wDataSize);
	}
	case DBR_GR_INVENTORYCONSUMPTIONINFO:
	{
		return OnDBPCInventoryConsumptionResoult(dwContextID, pData, wDataSize);
	}
	}
...
bool CAttemperEngineSink::SendRoomMessage(LPCTSTR lpszMessage, WORD wType)
{
	...
	DWORD wSendSize=wHeadSize+CountStringBuffer(SystemMessage.szString);
...
//发送数据
bool CAttemperEngineSink::SendData(BYTE cbSendMask, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)
{
	//机器数据
...
//发送数据
bool CAttemperEngineSink::SendData(DWORD dwSocketID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)
{
	//发送数据
...
//发送数据
bool CAttemperEngineSink::SendData(IServerUserItem * pIServerUserItem, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)
{
	//效验参数
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem==NULL) return false;
...
bool CAttemperEngineSink::OnEventUserItemScore(IServerUserItem * pIServerUserItem, BYTE cbReason)
{
	...
	if ((CServerRule::IsImmediateWriteScore(m_pGameServiceOption->dwServerRule)==true)&&(pIServerUserItem->IsVariation()==true))
	{
		...
		//mChen add, not change Score in GameScoreInfo 
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	UserInfo.dwPlayCount = pDBOLogonSuccess->dwPlayCount;
	UserInfo.RegisterDate = pDBOLogonSuccess->RegisterDate;

	////mChen log
	//TCHAR szString[512] = TEXT("");
	//_sntprintf(szString, CountArray(szString), TEXT("OnDBLogonSuccess::szHeadHttp=%s"), UserInfo.szHeadHttp);
	////提示消息
	//CTraceService::TraceString(szString, TraceLevel_Normal);
...
bool CAttemperEngineSink::OnTCPNetworkMainGame(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	switch (wSubCmdID)
	{
		case SUB_C_AWARD_DONE:
		{
			return OnTCPNetworkSubPCAwardDone(pData, wDataSize, dwSocketID);
		}
		case SUB_C_CONSUMPTION_INVENTORY:
		{
			return OnTCPNetworkSubPCInventoryConsumption(pData, wDataSize, dwSocketID);
		}
		default:
		{
			//消息处理 
...
{
	//Log
	//CTraceService::TraceString(TEXT("OnTCPNetworkSubUserStandUp1"), TraceLevel_Normal);
	...
	//Log
	//CTraceService::TraceString(TEXT("OnTCPNetworkSubUserStandUp2"), TraceLevel_Normal);
	...
	//Log
	//CTraceService::TraceString(TEXT("OnTCPNetworkSubUserStandUp3"), TraceLevel_Normal);
	...
	{
		//掉线通知
		if(m_pIPrivateServiceManager!=NULL) m_pIPrivateServiceManager->OnEventReqStandUP(pIServerUserItem, pUserStandUp->cbForceLeave);
...
//奖励执行
bool CAttemperEngineSink::OnTCPNetworkSubPCAwardDone(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//参数校验
	ASSERT(wDataSize == sizeof(CMD_C_AwardDone));
	if (wDataSize != sizeof(CMD_C_AwardDone)) return false;

	//提取数据
	CMD_C_AwardDone *pAwardDone = (CMD_C_AwardDone *)pData;
	pAwardDone->szPassword[CountArray(pAwardDone->szPassword) - 1] = 0;

	//构造结构
	DBR_C_AwardDone AwardDone;
	AwardDone.dwUserID = pAwardDone->dwUserID;
	AwardDone.dwAwardGold = pAwardDone->dwAwardGold;
	AwardDone.cbCostType = pAwardDone->cbCostType;

	lstrcpyn(AwardDone.szPassword, pAwardDone->szPassword, CountArray(AwardDone.szPassword));

	//投递请求
	m_pIKernelDataBaseEngine->PostDataBaseRequest(DBR_GR_AWARD_DONE, dwSocketID, &AwardDone, sizeof(AwardDone));

	return true;
}
//奖励结果 WQ
bool CAttemperEngineSink::OnDBPCUserAwardResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//变量定义
	WORD wBindIndex = LOWORD(dwContextID);
	tagBindParameter * pBindParameter = GetBindParameter(wBindIndex);
	...
	//变量定义
	CMD_S_AwardResult * pAwardResult = (CMD_S_AwardResult *)pData;

	//用户效验
	ASSERT(pBindParameter->pIServerUserItem != NULL);
	if (pBindParameter->pIServerUserItem == NULL) return false;

	//处理过虑
	WORD wTableID = pBindParameter->pIServerUserItem->GetTableID();
	WORD wChairID = pBindParameter->pIServerUserItem->GetChairID();
	if ((wTableID == INVALID_TABLE) || (wChairID == INVALID_CHAIR)) return true;

	CTableFrame * pTableFrame = m_TableFrameArray[wTableID];
	pTableFrame->SendTableData(wChairID, SUB_S_AWARD_RESULT, pData, sizeof(CMD_S_AwardResult));

	return true;
}
//道具消费
bool CAttemperEngineSink::OnTCPNetworkSubPCInventoryConsumption(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//效验参数
	ASSERT(wDataSize == sizeof(CMD_C_InventoryConsumptionInfo));
	if (wDataSize != sizeof(CMD_C_InventoryConsumptionInfo)) return false;

	//处理消息
	CMD_C_InventoryConsumptionInfo * pInventoryConsumptionInfo = (CMD_C_InventoryConsumptionInfo *)pData;

	//变量定义
	DBR_C_InventoryConsumptionInfo InventoryConsumptionInfo;
	ZeroMemory(&InventoryConsumptionInfo, sizeof(InventoryConsumptionInfo));

	//构造数据
	InventoryConsumptionInfo.dwUserID = pInventoryConsumptionInfo->dwUserID;
	InventoryConsumptionInfo.cbItemID = pInventoryConsumptionInfo->cbItemID;
	InventoryConsumptionInfo.wAmount = pInventoryConsumptionInfo->wAmount;
	InventoryConsumptionInfo.cbCostType = pInventoryConsumptionInfo->cbCostType;

	//投递请求
	m_pIKernelDataBaseEngine->PostDataBaseRequest(DBR_GR_INVENTORYCONSUMPTIONINFO, dwSocketID, &InventoryConsumptionInfo, sizeof(InventoryConsumptionInfo));
	return true;
}
//道具消耗结果
bool CAttemperEngineSink::OnDBPCInventoryConsumptionResoult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//变量定义
	WORD wBindIndex = LOWORD(dwContextID);
	tagBindParameter * pBindParameter = GetBindParameter(wBindIndex);
	...
	//变量定义
	CMD_S_InventoryConsumptionInfoResult * pInventoryConsumptionInfoResult = (CMD_S_InventoryConsumptionInfoResult *)pData;

	IServerUserItem * pIServerUserItem = m_ServerUserManager.SearchUserItem(pInventoryConsumptionInfoResult->dwUserID);
	...
	CTableFrame * pTableFrame = m_TableFrameArray[wTableID];
	pTableFrame->SendTableData(wChairID, SUB_S_CONSUMPTION_INVENTORY_RESULT, pData, sizeof(CMD_S_InventoryConsumptionInfoResult));
	
	if (pInventoryConsumptionInfoResult->cbSuccess == 0)
	{
		...
		InventoryConsumptionEvent.cbItemID = pInventoryConsumptionInfoResult->cbItemID;

		//道具处理
		if (InventoryConsumptionEvent.cbItemID == InventoryItemID::ChangeModel)  //使用变身道具随机生成modelIndex
		{
			std::random_device rd;
			std::mt19937 mt(rd());
			InventoryConsumptionEvent.dwModelIndex = mt() % 0xFF;
			pBindParameter->pIServerUserItem->SetModelIndex(InventoryConsumptionEvent.dwModelIndex);

		}
		else if (InventoryConsumptionEvent.cbItemID == InventoryItemID::Resurrection)  //使用复活道具
		{
			pTableFrame->ResurrectionEvent(wChairID);
			pTableFrame->StealthEvent(5, wChairID);
		}
		else if (InventoryConsumptionEvent.cbItemID == InventoryItemID::Stealth)
		{
			pTableFrame->StealthEvent(15, wChairID);
		}
		pTableFrame->SendTableData(INVALID_CHAIR, SUB_S_CONSUMPTION_INVENTORY_EVENT, &InventoryConsumptionEvent, sizeof(InventoryConsumptionEvent));
		pTableFrame->SendLookonData(INVALID_CHAIR, SUB_S_CONSUMPTION_INVENTORY_EVENT, &InventoryConsumptionEvent, sizeof(InventoryConsumptionEvent));
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubQueryOption(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//模式规则
	//mChen
	if ((m_pGameServiceOption->wServerType&(GAME_GENRE_GOLD | GAME_GENRE_MATCH | GAME_GENRE_EDUCATE)) == 0) OptionCurrent.dwRuleMask |= SR_RECORD_GAME_SCORE;
	if ((m_pGameServiceOption->wServerType&(GAME_GENRE_GOLD | GAME_GENRE_MATCH | GAME_GENRE_EDUCATE)) == 0) OptionCurrent.dwRuleMask |= SR_IMMEDIATE_WRITE_SCORE;
...
bool CAttemperEngineSink::OnTCPNetworkSubOptionServer(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//记录成绩
	if ((m_pGameServiceOption->wServerType&(GAME_GENRE_GOLD|GAME_GENRE_MATCH))==0)//mChen
	{
...
VOID CAttemperEngineSink::OnEventUserLogon(IServerUserItem * pIServerUserItem, bool bAlreadyOnLine)
{
	...
	if (pIServerUserItem->IsMobileUser()==false)
	{
		...
		LogonSuccess.cbIsForAppleReview = 1;
		...
		DWORD wSendSize=wHeadSize+TableInfo.wTableCount*sizeof(TableInfo.TableStatusArray[0]);
...
VOID CAttemperEngineSink::OnEventUserLogout(IServerUserItem * pIServerUserItem, DWORD dwLeaveReason)
{
	...
	_sntprintf(szString, CountArray(szString), TEXT("OnEventUserLogout:remove from GameScoreLocker: dwUserID=%d, dwLeaveReason=%d"), pIServerUserItem->GetUserID(), dwLeaveReason);
	CTraceService::TraceString(szString, TraceLevel_Normal);
...
bool CAttemperEngineSink::PerformUnlockScore(DWORD dwUserID, DWORD dwInoutIndex, DWORD dwLeaveReason)
{
	...
	// Log
	TCHAR szString[128] = TEXT("");
	_sntprintf(szString, CountArray(szString), TEXT("PerformUnlockScore:remove from GameScoreLocker: dwUserID=%d, dwLeaveReason=%d"), dwUserID, dwLeaveReason);
	CTraceService::TraceString(szString, TraceLevel_Normal);
...
bool CAttemperEngineSink::PerformCheckVersion(DWORD dwPlazaVersion, DWORD dwFrameVersion, DWORD dwClientVersion, DWORD dwSocketID)
{
	...
	BYTE cbProductVer = GetProductVer(dwClientVersion);
	BYTE cbMainVer = GetMainVer(dwClientVersion);
	BYTE cbGetSubVer = GetSubVer(dwClientVersion);

	BYTE cbProductVer1 = GetProductVer(m_pGameServiceAttrib->dwClientVersion);
	BYTE cbMainVer1 = GetMainVer(m_pGameServiceAttrib->dwClientVersion);
	BYTE cbGetSubVer1 = GetSubVer(m_pGameServiceAttrib->dwClientVersion);
...
bool CAttemperEngineSink::SwitchUserItemConnect(IServerUserItem * pIServerUserItem, TCHAR szMachineID[LEN_MACHINE_ID], WORD wTargetIndex,BYTE cbDeviceType,WORD wBehaviorFlags,WORD wPageTableCount)
{
	...
	if (pIServerUserItem->GetBindIndex()!=INVALID_WORD)
	{
		...
		// Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("SwitchUserItemConnect::您的帐号在另一地方进入了此游戏房间，您被迫离开: userName=%s, userId=%d, chairId=%d"), pIServerUserItem->GetNickName(), pIServerUserItem->GetUserInfo()->dwUserID, pIServerUserItem->GetChairID());
	...
	if (pIServerUserItem->GetUserStatus()==US_OFFLINE)
	{
		...
		BYTE cbLastUserStatus = pIServerUserItem->GetLastUserStatus();
		pIServerUserItem->SetUserStatus(cbLastUserStatus, wTableID, wChairID);
	...
	if ((bAndroidUser==false)&&(bIsOffLine==false)&&(bSameMachineID==false))
	{
		...
		// Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("SwitchUserItemConnect::您的帐号在另一地方进入了此游戏房间，对方被迫离开: userName=%s, userId=%d, chairId=%d"), pIServerUserItem->GetNickName(), pIServerUserItem->GetUserInfo()->dwUserID, pIServerUserItem->GetChairID());
		CTraceService::TraceString(szString, TraceLevel_Warning);
	}
...
bool CAttemperEngineSink::SendUserInfoPacket(IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	...
	//mChen add
	pUserInfoHead->dwClientAddr = pUserInfo->dwClientAddr;
	pUserInfoHead->dwPlayCount = pUserInfo->dwPlayCount;
	pUserInfoHead->RegisterDate = pUserInfo->RegisterDate;

	//mChen add, for HideSeek
	pUserInfoHead->cbTeamType = pUserInfo->cbTeamType;
	pUserInfoHead->cbModelIndex = pUserInfo->cbModelIndex;
	...
	//mChen add, for headPic
	SendPacket.AddPacket(pUserInfo->szHeadHttp, DTP_GR_HEAD_HTTP);
...
//群发数据
bool CAttemperEngineSink::SendDataBatchToMobileUser(WORD wCmdTable, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)
{
	//枚举用户
	WORD wEnumIndex=0;
