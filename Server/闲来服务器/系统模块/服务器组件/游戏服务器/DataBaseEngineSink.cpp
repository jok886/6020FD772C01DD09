...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GR_LOAD_SENSITIVE_WORDS://加载敏感词
		...
		//mChen add
	case DBR_GR_MATCH_SIGNUP_CHECK:			//报名查询
		{
			bSucceed = OnRequestMatchSignupCheck(dwContextID, pData, wDataSize, dwUserID);
		}
		break;
	...
	case DBR_GR_MATCH_REWARD:			//比赛奖励
		...
		break;	
	case DBR_GR_MATCH_RECORD_SCORE:			//比赛积分
	{
		bSucceed = OnRequestMatchRecordScore(dwContextID, pData, wDataSize, dwUserID);
	}
	break;
	case DBR_GR_PRIVATE_INFO:			//比赛奖励
		...
		break;	
	case DBR_GR_JOIN_FAILED:			//加入房间失败，重置GameScoreLocker
		{
			bSucceed = OnRequestJoinFailed(dwContextID, pData, wDataSize, dwUserID);
		}
		break;
	case DBR_GR_CREATE_PRIVAT:			//检查是否可以创建房间
		...
		break;
	case DBR_GR_JOIN_CHECK:			//检查是否可以加入AA房间
		{
			bSucceed = OnRequestCreatePrivate(dwContextID, pData, wDataSize, dwUserID);
		}
		break;
	case DBR_GR_CREATE_PRIVAT_COST:			//扣除房费
		...
		//mChen add,for代开房
	case DBR_GR_INSERT_PRIVSTE_ROOM:
		{
			bSucceed = OnRequestInsertPrivateRoom(dwContextID, pData, wDataSize, dwUserID);
		}
		break;

		//mChen add, for HideSeek
		//删除锁表记录
	case DBR_GR_CLEAR_GAME_SCORE_LOCKER:
		{
			bSucceed = OnRequestClearGameScoreLocker(dwContextID, pData, wDataSize, dwUserID);
		}
	case DBR_GR_AWARD_DONE:
	{
		bSucceed = OnRequestaAwardDone(dwContextID, pData, wDataSize, dwUserID);
	}
	break;
	case DBR_GR_INVENTORYCONSUMPTIONINFO:
	{
		bSucceed = OnRequestInventoryConsumption(dwContextID, pData, wDataSize);
	}
	break;
	}
...
bool CDataBaseEngineSink::OnRequestGameScoreRecord(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	try
	{
		...
		//mChen comment
		//m_GameDBAide.AddParameter(TEXT("@dataUserDefine"),(BYTE*)&pGameScoreRecord->dataGameDefine[0],(ULONG)pGameScoreRecord->dataGameDefine.size());
	...
	try
	{
		...
		if (lResultCode==DB_SUCCESS)
		{
			...
			for (WORD i=0;i<(WORD)kTotalRecord.kUserID.size();i++)
			{
				...
				if (kTotalRecord.kUserID[i] != 0)//mChen add, 去除无效用户
				{
					//房间信息
...
//mChen add
//比赛报名查询
bool CDataBaseEngineSink::OnRequestMatchSignupCheck(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	//效验参数
	ASSERT(wDataSize == sizeof(DBR_GR_MatchSignupCheck));
	if (wDataSize != sizeof(DBR_GR_MatchSignupCheck)) return false;

	//变量定义
	DBR_GR_MatchSignupCheck * pMatchSignupCheck = (DBR_GR_MatchSignupCheck *)pData;
	dwUserID = pMatchSignupCheck->dwUserID;
	...
	try
	{
		//转化地址
		TCHAR szClientAddr[16] = TEXT("");
		BYTE * pClientAddr = (BYTE *)&pMatchSignupCheck->dwClientAddr;
		_sntprintf(szClientAddr, CountArray(szClientAddr), TEXT("%d.%d.%d.%d"), pClientAddr[0], pClientAddr[1], pClientAddr[2], pClientAddr[3]);

		//构造参数
		m_GameDBAide.ResetParameter();
		m_GameDBAide.AddParameter(TEXT("@dwUserID"), pMatchSignupCheck->dwUserID);
		//m_GameDBAide.AddParameter(TEXT("@strPassword"), pMatchSignupCheck->szPassword);
		m_GameDBAide.AddParameter(TEXT("@strClientIP"), szClientAddr);
		m_GameDBAide.AddParameter(TEXT("@strMachineID"), pMatchSignupCheck->szMachineID);
		m_GameDBAide.AddParameter(TEXT("@wKindID"), m_pGameServiceOption->wKindID);
		m_GameDBAide.AddParameter(TEXT("@wServerID"), m_pGameServiceOption->wServerID);
		...
		if ((m_pGameServiceOption->wServerType&GAME_GENRE_MATCH) != 0)
		{
			m_GameDBAide.AddParameter(TEXT("@dwMatchID"), pMatchSignupCheck->dwMatchID);
			m_GameDBAide.AddParameter(TEXT("@dwMatchNO"), pMatchSignupCheck->dwMatchNO);
		}
		...
		//执行查询
		LONG lResultCode = m_GameDBAide.ExecuteProcess(TEXT("GSP_GR_SignupCheck"), true);

		//构造结构
		DBO_GR_MatchSingupCheckResult MatchSignupCheckResult;
		ZeroMemory(&MatchSignupCheckResult, sizeof(MatchSignupCheckResult));

		//设置变量
		MatchSignupCheckResult.bResultCode = lResultCode == DB_SUCCESS;
		MatchSignupCheckResult.bInGamePhase = pMatchSignupCheck->bInGamePhase;
		//if (!MatchSignupCheckResult.bResultCode)
		{
			// 错误描述
			CDBVarValue DBVarValue;
			m_GameDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
			lstrcpyn(MatchSignupCheckResult.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(MatchSignupCheckResult.szDescribeString));
		}

		//发送结果
		WORD wSendDataSize = sizeof(MatchSignupCheckResult) - sizeof(MatchSignupCheckResult.szDescribeString);
		wSendDataSize += CountStringBuffer(MatchSignupCheckResult.szDescribeString);
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_MATCH_SIGNUP_CHECK_RESULT, dwContextID, &MatchSignupCheckResult, wSendDataSize);
	...
	catch (IDataBaseException * pIException)
	{
		...
		//构造结构
		DBO_GR_MatchSingupResult MatchSignupCheckResult;
		ZeroMemory(&MatchSignupCheckResult, sizeof(MatchSignupCheckResult));

		//设置变量		
		MatchSignupCheckResult.bResultCode = false;
		lstrcpyn(MatchSignupCheckResult.szDescribeString, TEXT("由于数据库操作异常，请您稍后重试！"), CountArray(MatchSignupCheckResult.szDescribeString));
...
bool CDataBaseEngineSink::OnRequestMatchOver(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	try
	{
		...
		if(lReturnValue==DB_SUCCESS)
		{
			...
			//读取记录
			//mChen edit
			while(m_GameDBModule->IsRecordsetEnd()==false)//for (WORD i = 0; i < 4; i++)//
			{
...
//比赛积分
bool CDataBaseEngineSink::OnRequestMatchRecordScore(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	//效验参数
	ASSERT(wDataSize == sizeof(DBR_GR_MatchRecordScore));
	if (wDataSize != sizeof(DBR_GR_MatchRecordScore)) return false;

	//变量定义
	DBR_GR_MatchRecordScore * pMatchScore = (DBR_GR_MatchRecordScore *)pData;
	dwUserID = pMatchScore->dwUserID;
	...
	try
	{
		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pMatchScore->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@strNickName"), pMatchScore->strNickName);
		m_TreasureDBAide.AddParameter(TEXT("@lScore"), pMatchScore->lScore);
		
		//结果处理
		m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_MatchRecordScore"), false);
		return true;
	}
	catch (IDataBaseException * pIException)
	{
...
bool CDataBaseEngineSink::OnRequestPrivateInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	try
	{
		...
		if (!lReturnValue)
		{
			...
			kPrivateInfo.lPlayAvgCost[0] = m_PlatformDBAide.GetValue_LONG(TEXT("PlayCostAvg1"));
			kPrivateInfo.lPlayAvgCost[1] = m_PlatformDBAide.GetValue_LONG(TEXT("PlayCostAvg2"));
			kPrivateInfo.lPlayAvgCost[2] = m_PlatformDBAide.GetValue_LONG(TEXT("PlayCostAvg3"));
			kPrivateInfo.lPlayAvgCost[3] = m_PlatformDBAide.GetValue_LONG(TEXT("PlayCostAvg4"));

			//mChen add,金币场模拟比赛场
			kPrivateInfo.cbMatchPlayCout = m_PlatformDBAide.GetValue_BYTE(TEXT("MatchPlayCout"));
			m_PlatformDBAide.GetValue_SystemTime(TEXT("MatchStartTime"), kPrivateInfo.MatchStartTime);
			m_PlatformDBAide.GetValue_SystemTime(TEXT("MatchEndTime"), kPrivateInfo.MatchEndTime);
...
bool CDataBaseEngineSink::OnRequestJoinFailed(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	//效验参数
	ASSERT(wDataSize == sizeof(DBR_GR_Jion_Failed));
	if (wDataSize != sizeof(DBR_GR_Jion_Failed)) return false;

	//变量定义
	DBR_GR_Jion_Failed * pPrivateInfo = (DBR_GR_Jion_Failed *)pData;
	...
	try
	{
		...
		m_TreasureDBAide.AddParameter(TEXT("@wServerID"), pPrivateInfo->wServerID);
		
		// Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("OnRequestJoinFailed:remove from GameScoreLocker: dwUserID=%d"), pPrivateInfo->dwUserID);
		CTraceService::TraceString(szString, TraceLevel_Warning);

		LONG lReturnValue = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_JoinFalied"), false);
		return true;
	}
	catch (IDataBaseException * pIException)
	{
...
bool CDataBaseEngineSink::OnRequestCheckJionPrivate(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	//效验参数
	...
	try
	{
		...
		//mChen add
		kCreatePrivate.cbPlayCostTypeIdex = pPrivateInfo->cbPlayCostTypeIdex;
		kCreatePrivate.lBaseScore = pPrivateInfo->lBaseScore;
		//ZY add
		kCreatePrivate.PlayerCount = pPrivateInfo->PlayerCount;
		...
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_CHECK_JOIN_PRIVATE, dwContextID, &kCreatePrivate, sizeof(kCreatePrivate));
...
bool CDataBaseEngineSink::OnRequestCreatePrivate(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	try
	{
		...
		//mChen add, for HideSeek
		if (pPrivateInfo->cbRoomType == 0)//Type_Private
		{
			kCreatePrivate.cbChoosedMapIndex = pPrivateInfo->cbChoosedMapIndex;
		}
...
//mChen add, for HideSeek
//删除锁表记录
bool CDataBaseEngineSink::OnRequestClearGameScoreLocker(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	//请求处理
	try
	{
		...
		LONG lReturnValue = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_ClearGameScoreLocker"), true);
		return true;
	}
	catch (IDataBaseException * pIException)
	{
...
//WQ
bool CDataBaseEngineSink::OnRequestaAwardDone(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{

	try
	{
		//效验参数
		ASSERT(wDataSize == sizeof(DBR_C_AwardDone));
		if (wDataSize != sizeof(DBR_C_AwardDone)) return false;

		//请求处理
		DBR_C_AwardDone * pAwardDone = (DBR_C_AwardDone *)pData;

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pAwardDone->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@dwRaffleGold"), pAwardDone->dwAwardGold);
		m_TreasureDBAide.AddParameter(TEXT("@cbCostType"), pAwardDone->cbCostType);
		m_TreasureDBAide.AddParameter(TEXT("@strPassword"), pAwardDone->szPassword);
		...
		//执行脚本
		LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_AwardDone"), true);

		//变量定义
		CMD_S_AwardResult AwardResult;
		ZeroMemory(&AwardResult, sizeof(AwardResult));

		//读取数据
		if (lResultCode == DB_SUCCESS && m_TreasureDBModule->IsRecordsetEnd() == false)
		{
			AwardResult.dwUserID = pAwardDone->dwUserID;
			AwardResult.lScore = m_TreasureDBAide.GetValue_LONGLONG(TEXT("Score"));
			AwardResult.cbCostType = m_TreasureDBAide.GetValue_BYTE(TEXT("CostType"));
		}
		...
		AwardResult.bSuccessed = lResultCode;
		lstrcpyn(AwardResult.szNotifyContent, CW2CT(DBVarValue.bstrVal), CountArray(AwardResult.szNotifyContent));

		//发送结果
		DWORD wSendSize = sizeof(AwardResult) - sizeof(AwardResult.szNotifyContent) + CountStringBuffer(AwardResult.szNotifyContent);
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GR_AWARD_DONE, dwContextID, &AwardResult, wSendSize);
	...
	catch (IDataBaseException * pIException)
	{
		...
		//构造结构
		CMD_S_AwardResult AwardResult;
		AwardResult.bSuccessed = false;
		lstrcpyn(AwardResult.szNotifyContent, TEXT("由于数据库操作异常，请您稍后重试！"), CountArray(AwardResult.szNotifyContent));
...
//道具消费 WQ
bool CDataBaseEngineSink::OnRequestInventoryConsumption(DWORD dwContextID, void* pData, WORD wDataSize)
{
	try
	{
		DBR_C_InventoryConsumptionInfo *pInventoryConsumptionInfo = (DBR_C_InventoryConsumptionInfo *)pData;


		//构造参数
		m_TreasureDBAide.ResetParameter();

		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pInventoryConsumptionInfo->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@cbItemID"), pInventoryConsumptionInfo->cbItemID);
		m_TreasureDBAide.AddParameter(TEXT("@wAmount"), pInventoryConsumptionInfo->wAmount);
		m_TreasureDBAide.AddParameter(TEXT("@cbCostType"), pInventoryConsumptionInfo->cbCostType);
		//输出变量
		...
		//执行查询
		LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_InventoryConsumptionInfo"), true);
		//结果处理
		CDBVarValue DBVarValue;
		m_TreasureDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
		//变量定义
		CMD_S_InventoryConsumptionInfoResult inventoryconsumptioninforesult;
		ZeroMemory(&inventoryconsumptioninforesult, sizeof(inventoryconsumptioninforesult));

		//构造变量
		if (lResultCode == DB_SUCCESS)
		{
			inventoryconsumptioninforesult.cbSuccess = DB_SUCCESS;
			inventoryconsumptioninforesult.cbItemID = m_TreasureDBAide.GetValue_BYTE(TEXT("ItemID"));
			inventoryconsumptioninforesult.dwFinalScore = m_TreasureDBAide.GetValue_DWORD(TEXT("FinalScore"));
			inventoryconsumptioninforesult.cbCostType = m_TreasureDBAide.GetValue_BYTE(TEXT("CostType"));

		}
		else
		{
			inventoryconsumptioninforesult.cbSuccess = DB_ERROR;
			
		}
		//获取描述信息
		lstrcpyn(inventoryconsumptioninforesult.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(inventoryconsumptioninforesult.szDescribeString));
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GR_INVENTORYCONSUMPTIONINFO, dwContextID, &inventoryconsumptioninforesult, sizeof(inventoryconsumptioninforesult));
	}
	catch (IDataBaseException* pIException)
	{
		//输出错误
...
//mChen add,for代开房
bool CDataBaseEngineSink::OnRequestInsertPrivateRoom(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{

	//效验参数
	ASSERT(wDataSize == sizeof(DBR_GR_Insert_Private_Room));
	if (wDataSize != sizeof(DBR_GR_Insert_Private_Room)) return false;

	//变量定义
	DBR_GR_Insert_Private_Room * pPrivateRoomInfo = (DBR_GR_Insert_Private_Room *)pData;
	...
	try
	{
		//构造参数
		m_PlatformDBAide.ResetParameter();
		m_PlatformDBAide.AddParameter(TEXT("@dwUserID"), pPrivateRoomInfo->dwUserID);
		m_PlatformDBAide.AddParameter(TEXT("@dwRoomNum"), pPrivateRoomInfo->dwRoomNum);
		m_PlatformDBAide.AddParameter(TEXT("@dwBaseScore"), pPrivateRoomInfo->dwBaseScore);
		m_PlatformDBAide.AddParameter(TEXT("@dwPlayCout"), pPrivateRoomInfo->dwPlayCout);
		m_PlatformDBAide.AddParameter(TEXT("@dwChairCount"), pPrivateRoomInfo->dwChairCount);

		TCHAR szDescribeString[128] = TEXT("");
		m_PlatformDBAide.AddParameterOutput(TEXT("@strErrorDescribe"), szDescribeString, sizeof(szDescribeString), adParamOutput);

		LONG lReturnValue = m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GR_InsertUserRoom"), true);

		//错误描述
		CDBVarValue DBVarValue;
		TCHAR	szDescResult[128];
		m_PlatformDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
		lstrcpyn(szDescResult, CW2CT(DBVarValue.bstrVal), CountArray(szDescResult));
...
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient,BYTE cbDeviceType,WORD wBehaviorFlags,WORD wPageTableCount)
{
	if (dwErrorCode==DB_SUCCESS)
	{
		...
		m_LogonSuccess.dwPlayCount = m_GameDBAide.GetValue_DWORD(TEXT("PlayCount"));
		m_GameDBAide.GetValue_SystemTime(TEXT("RegisterDate"), m_LogonSuccess.RegisterDate);

		////mChen log
		//TCHAR szString[512] = TEXT("");
		//_sntprintf(szString, CountArray(szString), TEXT("OnLogonDisposeResult::szHeadHttp=%s"), m_LogonSuccess.szHeadHttp);
		////提示消息
		//CTraceService::TraceString(szString, TraceLevel_Normal);

		//用户资料
		//mChen hack
		BYTE cbGender = m_GameDBAide.GetValue_BYTE(TEXT("Gender"));
		if (cbGender == 0)
		{
			m_LogonSuccess.cbGender = 0;
		}
		else
		{
			m_LogonSuccess.cbGender = 1;
		}
