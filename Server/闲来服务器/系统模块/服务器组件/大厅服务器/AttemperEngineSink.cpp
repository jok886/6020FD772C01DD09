...
bool CAttemperEngineSink::OnEventTCPSocketLink(WORD wServiceID, INT nErrorCode)
{
	...
	if (wServiceID==NETWORK_CORRESPOND)
	{
		...
		//提示消息
		CTraceService::TraceString(TEXT("正在注册游戏大厅服务器..."),TraceLevel_Normal);

		//变量定义
		CMD_CS_C_RegisterLobby RegisterLobby;
		ZeroMemory(&RegisterLobby,sizeof(RegisterLobby));

		//mChen add, for HideSeek
		RegisterLobby.wLobbyPort = m_pInitParameter->m_wServicePort;

		//设置变量
		lstrcpyn(RegisterLobby.szServerName,m_pInitParameter->m_szServerName,CountArray(RegisterLobby.szServerName));
		lstrcpyn(RegisterLobby.szServerAddr,m_pInitParameter->m_ServiceAddress.szAddress,CountArray(RegisterLobby.szServerAddr));

		//发送数据
		m_pITCPSocketService->SendData(MDM_CS_REGISTER,SUB_CS_C_REGISTER_LOBBY,&RegisterLobby,sizeof(RegisterLobby));
...
bool CAttemperEngineSink::OnTCPSocketMainManagerService(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	case SUB_GP_LOBBY_CHAT:  //大厅聊天
		{
			//发送数据
			m_pITCPNetworkEngine->SendDataBatch(MDM_GP_USER_SERVICE, SUB_GP_LOBBY_CHAT, pData, wDataSize, 0L);

			return true;
		}
	case SUB_CS_L_LOBBY_MESSAGE:  //大厅消息
		{
			//发送数据
			//m_pITCPNetworkEngine->SendDataBatch(MDM_GR_USER, ..., pData, wDataSize, BG_COMPUTER);
...
bool CAttemperEngineSink::OnTCPNetworkMainPCUserService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_GP_LOBBY_CHAT:			  //大厅聊天
	{
		OnTCPNetworkSubLobbyChat(wSubCmdID, pData, wDataSize, dwSocketID);
		return true;
	}
...
//mChen add, for HideSeek
bool CAttemperEngineSink::OnTCPNetworkSubLobbyChat(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	////参数校验
	//ASSERT(wDataSize == sizeof(CMD_GP_CHAT));
	//if (wDataSize != sizeof(CMD_GP_CHAT)) return false;

	//发送给协调服务器
	m_pITCPSocketService->SendData(MDM_CS_MANAGER_SERVICE, SUB_GP_LOBBY_CHAT, pData, wDataSize);
