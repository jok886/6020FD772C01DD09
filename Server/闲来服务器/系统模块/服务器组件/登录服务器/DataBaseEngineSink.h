...
protected:
	...
	//mChen add
	//增加/删除推荐人身份
	bool OnRequestAddDelSpreader(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//查询代理人列表
	bool OnRequestQuerySpreadersInfo(DWORD dwContextID, VOID * pData, WORD wDataSize);

	//mChen add
	//内购
	bool OnRequestAddPayment(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//名下用户交易信息
	bool OnRequestQueryChildrenPaymentInfo(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//企业提现
	bool OnRequestAddEnterprisePayment(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//企业提现结果
	bool OnRequestAddEnterprisePaymentResult(DWORD dwContextID, VOID * pData, WORD wDataSize);

	//mChen add, for HideSeek
	bool OnRequestBoughtTaggerModel(DWORD dwContextID, VOID * pData, WORD wDataSize);
...
protected:
	...
	//mChen add
	bool OnRequestRaffleDone(DWORD dwContextID, VOID * pData, WORD wDataSize);
...
protected:
   ...
   //商店购买 WQ
public:
	bool OnRequestAddShopItem(VOID * pData);
protected:
	bool OnRequestAddShopItem(DWORD dwContextID, VOID * pData, WORD wDataSize);
	bool OnRequestExchangeScoreInfo(DWORD dwContextID, VOID * pData, WORD wDataSize);
...
protected:
	...
	bool OnRequestTopMatchPlayers(DWORD dwContextID, VOID * pData, WORD wDataSize);//lin: add 查询本期比赛积分排行榜
	bool OnRequestNickNameByID(DWORD dwContextID, VOID * pData, WORD wDataSize);//lin:查询昵称，用于转房卡确认
	bool OnTransferDiamonds(DWORD dwContextID, VOID * pData, WORD wDataSize);//lin:转房卡
