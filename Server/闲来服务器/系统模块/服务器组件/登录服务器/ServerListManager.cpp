...
CServerListManager::CServerListManager()
{
	...
	m_LobbyItemMap.InitHashTable(PRIME_SERVER);
...
//mChen add, for HideSeek
//插入大厅
bool CServerListManager::InsertGameLobby(tagGameLobby * pGameLobby)
{
	//效验参数
	ASSERT(pGameLobby != NULL);
	if (pGameLobby == NULL) return false;

	tagGameLobby * pGameLobbyItem = NULL;
	if (m_LobbyItemMap.Lookup(pGameLobby->wLobbyID, pGameLobbyItem) == FALSE)
	{
		pGameLobbyItem = new tagGameLobby;
		if (pGameLobbyItem == NULL) return false;
	}

	CopyMemory(pGameLobbyItem, pGameLobby, sizeof(tagGameLobby));

	//设置索引
	m_LobbyItemMap[pGameLobby->wLobbyID] = pGameLobbyItem;
}
//删除大厅
bool CServerListManager::DeleteGameLobby(WORD wLobbyID)
{
	//查找房间
	tagGameLobby * pGamLobbyItem = NULL;
	if (m_LobbyItemMap.Lookup(wLobbyID, pGamLobbyItem) == FALSE) return false;

	//删除数据
	m_LobbyItemMap.RemoveKey(wLobbyID);

	SafeDelete(pGamLobbyItem);

	return false;
}
//清理大厅
VOID CServerListManager::ClearLobbyItem()
{
	//变量定义
	WORD wKey = 0;
	POSITION Position = NULL;

	tagGameLobby * pLobbyItem = NULL;
	Position = m_LobbyItemMap.GetStartPosition();
	while (Position != NULL)
	{
		m_LobbyItemMap.GetNextAssoc(Position, wKey, pLobbyItem);
		SafeDelete(pLobbyItem);
	}
	m_LobbyItemMap.RemoveAll();
}
//枚举大厅
tagGameLobby * CServerListManager::EmunGameLobbyItem(POSITION & Position)
{
	//变量定义
	WORD wKey = 0;
	tagGameLobby * pGameLobbyItem = NULL;
	...
	//获取对象
	if (Position == NULL) Position = m_LobbyItemMap.GetStartPosition();
	if (Position != NULL) m_LobbyItemMap.GetNextAssoc(Position, wKey, pGameLobbyItem);

	return pGameLobbyItem;
}
