...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GP_ADDDEL_SPREADER:
		{
			return OnRequestAddDelSpreader(dwContextID, pData, wDataSize);
		}
		//查询代理人列表
	case DBR_GP_QUERY_SPREADERS_INFO:
		{
			return OnRequestQuerySpreadersInfo(dwContextID, pData, wDataSize);
		}

		//mChen add
		//内购
	case DBR_GP_ADD_PAYMENT:
		{
			return OnRequestAddPayment(dwContextID, pData, wDataSize);
		}
		//名下用户交易信息
	case DBR_GP_QUERY_CHILDREN_PAYMENT_INFO:
		{
			return OnRequestQueryChildrenPaymentInfo(dwContextID, pData, wDataSize);
		}
		//企业提现
	case DBR_GP_ADD_ENTERPRISE_PAYMENT:
		{
			return OnRequestAddEnterprisePayment(dwContextID, pData, wDataSize);
		}
		//企业提现结果
	case DBR_GP_ADD_ENTERPRISE_PAYMENT_RESULT:
		{
			return OnRequestAddEnterprisePaymentResult(dwContextID, pData, wDataSize);
		}

		//mChen add, for HideSeek
	case DBR_GP_BOUGHT_TAGGER_MODEL:
	{
		return OnRequestBoughtTaggerModel(dwContextID, pData, wDataSize);
	}
	...
	case DBR_GP_CHECKIN_AWARD:			//执行签到
		...
		//mChen add
	case DBR_GP_RAFFLE_DONE:			//执行抽奖
		{
			return OnRequestRaffleDone(dwContextID, pData, wDataSize);
		}
	...
	case DBR_GP_QUERY_TOP_PLAYERS:
		{
			return OnRequestTopMatchPlayers(dwContextID, pData, wDataSize);
		}
	case DBR_GP_QUERY_NICKNAME:
		{
			return OnRequestNickNameByID(dwContextID, pData, wDataSize);;
		}
	case DBR_GP_TRANSFER_DIAMOND:
		{
			return OnTransferDiamonds(dwContextID, pData, wDataSize);
		}
	case DBR_GP_ADD_SHOPITEM:   //商品购买  WQ
	{
		//return OnRequestAddShopItem(dwContextID,pData,wDataSize);
		return OnRequestAddShopItem(pData);
	}
	case DBR_GP_ADD_EXCHANGSCOREINFO:
	{
		return OnRequestExchangeScoreInfo(dwContextID, pData, wDataSize);
	}
	}
...
bool CDataBaseEngineSink::OnRequestLogonAccounts(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		//不让游客登陆
		std::string strAccounts = pLogonAccounts->szAccounts;
		std::size_t pos = strAccounts.find("Account_");
		...
		m_AccountsDBAide.AddParameter(TEXT("@strUid"), pLogonAccounts->szUid);
		...
		m_AccountsDBAide.AddParameter(TEXT("@wKindID"), pLogonAccounts->wKindID);
...
bool CDataBaseEngineSink::OnRequestRegisterAccounts(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		m_AccountsDBAide.AddParameter(TEXT("@strUid"), pRegisterAccounts->szUid);
		...
		m_AccountsDBAide.AddParameter(TEXT("@dwSpreaderId"), pRegisterAccounts->dwSpreaderID);//mChen edit,m_AccountsDBAide.AddParameter(TEXT("@strSpreader"),pRegisterAccounts->szSpreader);
		...
		m_AccountsDBAide.AddParameter(TEXT("@wChannleID"), pRegisterAccounts->wChannleID);
		...
		//mChen add, for Match Time
		m_AccountsDBAide.AddParameter(TEXT("@wKindID"), pRegisterAccounts->wKindID);
...
bool CDataBaseEngineSink::OnRequestModifyMachine(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		if (m_AccountsDBAide.GetReturnValue()==DB_SUCCESS)
		{
			...
			DWORD wSendSize=sizeof(OperateSuccess)-sizeof(OperateSuccess.szDescribeString)+wStringSize;
		...
		else
		{
			...
			DWORD wSendSize=sizeof(OperateFailure)-sizeof(OperateFailure.szDescribeString)+wStringSize;
...
bool CDataBaseEngineSink::OnRequestModifySpreader(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		m_AccountsDBAide.AddParameter(TEXT("@dwSpreaderId"), pModifyUnderWrite->dwSpreaderID);//mChen edit,m_AccountsDBAide.AddParameter(TEXT("@strSpreader"),pModifyUnderWrite->szSpreader);
		m_AccountsDBAide.AddParameter(TEXT("@strClientIP"),szClientAddr);
		...
		//mChen add
		OperateSuccess.cbOperateType = 0;//操作类型：0-绑定代理，1-增加代理，2-删除代理
		OperateSuccess.dwBindSpreaderId = pModifyUnderWrite->dwSpreaderID;
...
//mChen add,增加/删除推荐人身份
bool CDataBaseEngineSink::OnRequestAddDelSpreader(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		ASSERT(wDataSize == sizeof(DBR_GP_AddDelSpreader));
		if (wDataSize != sizeof(DBR_GP_AddDelSpreader)) return false;

		//请求处理
		DBR_GP_AddDelSpreader * pAddDelSpreader = (DBR_GP_AddDelSpreader *)pData;

		//转化地址
		TCHAR szClientAddr[16] = TEXT("");
		BYTE * pClientAddr = (BYTE *)&pAddDelSpreader->dwClientAddr;
		_sntprintf(szClientAddr, CountArray(szClientAddr), TEXT("%d.%d.%d.%d"), pClientAddr[0], pClientAddr[1], pClientAddr[2], pClientAddr[3]);

		//构造参数
		m_AccountsDBAide.ResetParameter();
		m_AccountsDBAide.AddParameter(TEXT("@dwUserID"), pAddDelSpreader->dwUserID);
		m_AccountsDBAide.AddParameter(TEXT("@strPassword"), pAddDelSpreader->szPassword);

		m_AccountsDBAide.AddParameter(TEXT("@dwSpreaderId"), pAddDelSpreader->dwSpreaderID);

		m_AccountsDBAide.AddParameter(TEXT("@strSpreaderRealName"), pAddDelSpreader->szSpreaderRealName);
		///m_AccountsDBAide.AddParameter(TEXT("@strSpreaderIDCardNo"), pAddDelSpreader->szSpreaderIDCardNo);
		m_AccountsDBAide.AddParameter(TEXT("@strSpreaderTelNum"), pAddDelSpreader->szSpreaderTelNum);
		m_AccountsDBAide.AddParameter(TEXT("@strSpreaderWeiXinAccount"), pAddDelSpreader->szSpreaderWeiXinAccount);

		m_AccountsDBAide.AddParameter(TEXT("@wSpreaderLevel"), pAddDelSpreader->wSpreaderLevel);
		m_AccountsDBAide.AddParameter(TEXT("@dwParentSpreaderID"), pAddDelSpreader->dwParentSpreaderID);

		m_AccountsDBAide.AddParameter(TEXT("@bIsAddOperate"), pAddDelSpreader->bIsAddOperate);
		m_AccountsDBAide.AddParameter(TEXT("@strClientIP"), szClientAddr);
		...
		//执行查询
		LONG lResultCode = m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GP_AddDelSpreader"), true);
		...
		DBO_GP_SpreaderResoult OperateSuccess;
		ZeroMemory(&OperateSuccess, sizeof(OperateSuccess));
		OperateSuccess.cbOperateType = (pAddDelSpreader->bIsAddOperate ? 1:2);//mChen add, 操作类型：0-绑定代理，1-增加代理，2-删除代理
...
//查询代理人列表
bool CDataBaseEngineSink::OnRequestQuerySpreadersInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		ASSERT(wDataSize == sizeof(DBR_GP_QuerySpreadersInfo));
		if (wDataSize != sizeof(DBR_GP_QuerySpreadersInfo)) return false;

		//请求处理
		DBR_GP_QuerySpreadersInfo * pQuerySpreadersInfo = (DBR_GP_QuerySpreadersInfo *)pData;

		//构造参数
		m_AccountsDBAide.ResetParameter();
		m_AccountsDBAide.AddParameter(TEXT("@dwUserID"), pQuerySpreadersInfo->dwUserID);
		m_AccountsDBAide.AddParameter(TEXT("@strPassword"), pQuerySpreadersInfo->szPassword);
		...
		//执行查询
		LONG lResultCode = m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GP_QuerySpreadersInfo"), true);

		//结果处理
		CDBVarValue DBVarValue;
		m_AccountsDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);

		DBO_GP_SpreadersInfoResoult kGPNetInfo;
		ZeroMemory(&kGPNetInfo, sizeof(kGPNetInfo));

		//构造变量
		kGPNetInfo.lResultCode = lResultCode;
		kGPNetInfo.wPacketIdx = 0;
		lstrcpyn(kGPNetInfo.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(kGPNetInfo.szDescribeString));
		LONG recordCount = 0;
		if (lResultCode == DB_SUCCESS)//add to fix ADO 错误：0x800a0e78，对象关闭时，不允许操作。
		{
			recordCount = m_AccountsDBModule->GetRecordCount();
		}
		kGPNetInfo.wPacketNum = recordCount / Max_Spreaders_Num_PerSend;
		WORD wMod = recordCount % Max_Spreaders_Num_PerSend;
		if (wMod > 0)
		{
			kGPNetInfo.wPacketNum++;
		}
		WORD wItemCout = 0;
		if (lResultCode == DB_SUCCESS)
		{
			//读取记录
			while (m_AccountsDBModule->IsRecordsetEnd() == false)//for (WORD i = 0; i < 4; i++)//
			{
				kGPNetInfo.SpreaderInfoItems[wItemCout].SpreaderId = m_AccountsDBAide.GetValue_DWORD(TEXT("SpreaderId"));

				m_AccountsDBAide.GetValue_String(TEXT("RealName"), kGPNetInfo.SpreaderInfoItems[wItemCout].RealName, CountArray(kGPNetInfo.SpreaderInfoItems[wItemCout].RealName));
				//m_AccountsDBAide.GetValue_String(TEXT("IDCardNo"), kGPNetInfo.SpreaderInfoItems[wItemCout].IDCardNo, CountArray(kGPNetInfo.SpreaderInfoItems[wItemCout].IDCardNo));
				m_AccountsDBAide.GetValue_String(TEXT("TelNum"), kGPNetInfo.SpreaderInfoItems[wItemCout].TelNum, CountArray(kGPNetInfo.SpreaderInfoItems[wItemCout].TelNum));
				m_AccountsDBAide.GetValue_String(TEXT("WeiXinAccount"), kGPNetInfo.SpreaderInfoItems[wItemCout].WeiXinAccount, CountArray(kGPNetInfo.SpreaderInfoItems[wItemCout].WeiXinAccount));

				kGPNetInfo.SpreaderInfoItems[wItemCout].SpreaderLevel = m_AccountsDBAide.GetValue_WORD(TEXT("SpreaderLevel"));
				kGPNetInfo.SpreaderInfoItems[wItemCout].ParentID = m_AccountsDBAide.GetValue_DWORD(TEXT("ParentID"));
				wItemCout++;

				if (wItemCout >= Max_Spreaders_Num_PerSend)
				{
					kGPNetInfo.wItemCount = wItemCout;

					//发送结果
					DWORD itemSize = sizeof(SpreaderInfoItem);
					DWORD wDataSize = itemSize*wItemCout;
					DWORD wHeadSize = sizeof(kGPNetInfo) - sizeof(kGPNetInfo.SpreaderInfoItems);
					m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_SPREADERS_INFO_RESOULT, dwContextID, &kGPNetInfo, wHeadSize + wDataSize);

					kGPNetInfo.wPacketIdx++;
					wItemCout = 0;
				}
		...
		if (wItemCout > 0)
		{
			if (wItemCout != wMod)
			{
				...
				_sntprintf(szString, CountArray(szString), TEXT("OnRequestQuerySpreadersInfo error(wItemCout!=wMo): wItemCout=%d, wMod=%d"), wItemCout, wMod);
				CTraceService::TraceString(szString, TraceLevel_Warning);
			}

			if (kGPNetInfo.wPacketIdx != kGPNetInfo.wPacketNum - 1)
			{
				//log
				TCHAR szString[512] = TEXT("");
				_sntprintf(szString, CountArray(szString), TEXT("OnRequestQuerySpreadersInfo error(wPacketIdx!=wPacketNum-1): wPacketIdx=%d, wPacketNum=%d"), kGPNetInfo.wPacketIdx, kGPNetInfo.wPacketNum);
				CTraceService::TraceString(szString, TraceLevel_Warning);
			}
		...
		{
			kGPNetInfo.wPacketIdx = 0;
			kGPNetInfo.wItemCount = 0;
			kGPNetInfo.wPacketNum = 1;
...
//mChen add
//企业提现
bool CDataBaseEngineSink::OnRequestAddEnterprisePayment(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		...
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pAddEnterprisePayment->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@strPassword"), pAddEnterprisePayment->szPassword);
		m_TreasureDBAide.AddParameter(TEXT("@dwEnterprisePayment"), pAddEnterprisePayment->dwEnterprisePayment);

		//输出变量
		WCHAR szDescribe[256] = L"";
		m_TreasureDBAide.AddParameterOutput(TEXT("@strErrorDescribe"), szDescribe, sizeof(szDescribe), adParamOutput);

		//执行查询 
		LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_AddEnterprisePayment"), true);
		...
		//变量定义
		DBO_GP_AddEnterprisePaymentResoult kGPNetInfo;
		ZeroMemory(&kGPNetInfo, sizeof(kGPNetInfo));

		//构造变量
		kGPNetInfo.lResultCode = lResultCode;
		kGPNetInfo.dwEnterprisePayment = pAddEnterprisePayment->dwEnterprisePayment;
		lstrcpyn(kGPNetInfo.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(kGPNetInfo.szDescribeString));
		if (lResultCode == DB_SUCCESS)
		{
			m_TreasureDBAide.GetValue_String(TEXT("RealName"), kGPNetInfo.szRealName, CountArray(kGPNetInfo.szRealName));
			m_TreasureDBAide.GetValue_String(TEXT("Openid"), kGPNetInfo.szOpenid, CountArray(kGPNetInfo.szOpenid));
			kGPNetInfo.dwUserID = m_TreasureDBAide.GetValue_DWORD(TEXT("UserID"));
			kGPNetInfo.dwTotalLeftCash = m_TreasureDBAide.GetValue_DWORD(TEXT("TotalLeftCash"));
		}
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_ADD_ENTERPRISE_PAYMENT_RESULT, dwContextID, &kGPNetInfo, sizeof(kGPNetInfo));
		///OnOperateDisposeResult(dwContextID, lResultCode, CW2CT(DBVarValue.bstrVal), false);
...
//企业提现结果
bool CDataBaseEngineSink::OnRequestAddEnterprisePaymentResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{

	try
	{
		//效验参数
		ASSERT(wDataSize == sizeof(CMD_GP_AddEnterprisePaymentResult));
		if (wDataSize != sizeof(CMD_GP_AddEnterprisePaymentResult)) return false;

		//请求处理
		CMD_GP_AddEnterprisePaymentResult * pAddEnterprisePaymentResult = (CMD_GP_AddEnterprisePaymentResult *)pData;

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pAddEnterprisePaymentResult->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@dwEnterprisePayment"), pAddEnterprisePaymentResult->dwEnterprisePayment);
		...
		//执行查询 
		LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_AddEnterprisePaymentResult"), true);
		...
		////构造变量
		//kGPNetInfo.lResultCode = lResultCode;
		//kGPNetInfo.dwEnterprisePayment = pAddEnterprisePaymentResult->dwEnterprisePayment;
		//lstrcpyn(kGPNetInfo.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(kGPNetInfo.szDescribeString));
		//if (lResultCode == DB_SUCCESS)
		//{
		//	m_TreasureDBAide.GetValue_String(TEXT("RealName"), kGPNetInfo.szRealName, CountArray(kGPNetInfo.szRealName));
...
//mChen add, for HideSeek
bool CDataBaseEngineSink::OnRequestBoughtTaggerModel(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		...
		WORD wBoughtModelIndex = pBoughtTaggerModel->wBoughtModelIndex;
		BYTE cbStoreIdx = 0;
		while (wBoughtModelIndex >= 64)
		{
			wBoughtModelIndex -= 64;
			cbStoreIdx++;
		}
		INT64 lModelIndexToStore = 1 << wBoughtModelIndex;

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pBoughtTaggerModel->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@strPassword"), pBoughtTaggerModel->szPassword);
		m_TreasureDBAide.AddParameter(TEXT("@wBoughtModelIndex"), pBoughtTaggerModel->wBoughtModelIndex);
		m_TreasureDBAide.AddParameter(TEXT("@dwPayment"), pBoughtTaggerModel->dwPayment);
		m_TreasureDBAide.AddParameter(TEXT("@cbPaymentType"), pBoughtTaggerModel->cbPaymentType);
		m_TreasureDBAide.AddParameter(TEXT("@lModelIndexToStore"), lModelIndexToStore);
		...
		//执行查询 
		LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_BoughtTaggerModel"), true);
		...
		//变量定义
		DBO_GP_BoughtTaggerModelResoult kGPNetInfo;
		ZeroMemory(&kGPNetInfo, sizeof(kGPNetInfo));

		//构造变量
		kGPNetInfo.lResultCode = lResultCode;
		kGPNetInfo.wBoughtModelIndex = pBoughtTaggerModel->wBoughtModelIndex;
		lstrcpyn(kGPNetInfo.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(kGPNetInfo.szDescribeString));
		if (lResultCode == DB_SUCCESS)
		{
			kGPNetInfo.lUserScore = m_TreasureDBAide.GetValue_LONGLONG(TEXT("Score"));
			kGPNetInfo.lUserInsure = m_TreasureDBAide.GetValue_LONGLONG(TEXT("Insure"));
		}
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_BOUGHT_TAGGER_MODEL_RESULT, dwContextID, &kGPNetInfo, sizeof(kGPNetInfo));
		///OnOperateDisposeResult(dwContextID, lResultCode, CW2CT(DBVarValue.bstrVal), false);
...
//内购
bool CDataBaseEngineSink::OnRequestAddPayment(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		ASSERT(wDataSize == sizeof(DBR_GP_AddPayment));
		if (wDataSize != sizeof(DBR_GP_AddPayment)) return false;

		//请求处理
		DBR_GP_AddPayment * pAddPayment = (DBR_GP_AddPayment *)pData;

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pAddPayment->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@strPassword"), pAddPayment->szPassword);
		m_TreasureDBAide.AddParameter(TEXT("@dwPayment"), pAddPayment->dwPayment);
		m_TreasureDBAide.AddParameter(TEXT("@dwBoughtDiamond"), pAddPayment->dwBoughtDiamond);
		...
		//执行查询 
		LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_AddPayment"), true);
		...
		//变量定义
		DBO_GP_AddPaymentResoult kGPNetInfo;
		ZeroMemory(&kGPNetInfo, sizeof(kGPNetInfo));
		...
		if (lResultCode == DB_SUCCESS)
		{
			kGPNetInfo.dwFinalInsureScore = m_TreasureDBAide.GetValue_DWORD(TEXT("FinalInsureScore"));
		}
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_ADD_PAYMENT_RESULT, dwContextID, &kGPNetInfo, sizeof(kGPNetInfo));
		///OnOperateDisposeResult(dwContextID, lResultCode, CW2CT(DBVarValue.bstrVal), false);
...
//名下用户交易信息
bool CDataBaseEngineSink::OnRequestQueryChildrenPaymentInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		ASSERT(wDataSize == sizeof(DBR_GP_QueryChildrenPaymentInfo));
		if (wDataSize != sizeof(DBR_GP_QueryChildrenPaymentInfo)) return false;

		//请求处理
		DBR_GP_QueryChildrenPaymentInfo * pQueryChildrenPaymentInfo = (DBR_GP_QueryChildrenPaymentInfo *)pData;

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pQueryChildrenPaymentInfo->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@strPassword"), pQueryChildrenPaymentInfo->szPassword);
		...
		//执行查询
		LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_QueryChildrenPaymentInfo"), true);

		//结果处理
		CDBVarValue DBVarValue;
		m_TreasureDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);

		DBO_GP_ChildrenPaymentInfoResult kGPNetInfo;
		ZeroMemory(&kGPNetInfo, sizeof(kGPNetInfo));
		...
		if (lResultCode == DB_SUCCESS)//add to fix ADO 错误：0x800a0e78，对象关闭时，不允许操作。
		{
			recordCount = m_TreasureDBModule->GetRecordCount();
		}
		kGPNetInfo.wPacketNum = recordCount / Max_PaymentInfo_Num_PerSend;
		WORD wMod = recordCount % Max_PaymentInfo_Num_PerSend;
		if (wMod > 0)
		{
			kGPNetInfo.wPacketNum++;
		}
		...
		WORD wItemCout = 0;
		int nHasChildrenBuyCount = 0;
		if (lResultCode == DB_SUCCESS)//add to fix ADO 错误：0x800a0cc1，在对应所需名称或序数的集合中，未找到项目。
		{
			nHasChildrenBuyCount = m_TreasureDBAide.GetValue_DOUBLE(TEXT("HasChildrenBuyCount"));
			kGPNetInfo.dTotalGrantOfChildrenBuy = m_TreasureDBAide.GetValue_DOUBLE(TEXT("TotalGrantOfChildrenBuy"));
			kGPNetInfo.dExtraCash = m_TreasureDBAide.GetValue_DOUBLE(TEXT("ExtraCash"));
			kGPNetInfo.dCashedOut = m_TreasureDBAide.GetValue_DOUBLE(TEXT("CashedOut"));
			kGPNetInfo.dTotalLeftCash = m_TreasureDBAide.GetValue_DOUBLE(TEXT("TotalLeftCash"));
		}
		if (lResultCode == DB_SUCCESS && nHasChildrenBuyCount!=0)
		{
			//读取记录
			while (m_TreasureDBModule->IsRecordsetEnd() == false)
			{
				...
				kGPNetInfo.PaymentInfoItems[wItemCout].UserId = m_TreasureDBAide.GetValue_DWORD(TEXT("UserId"));
				kGPNetInfo.PaymentInfoItems[wItemCout].Payment = m_TreasureDBAide.GetValue_WORD(TEXT("Payment"));
				kGPNetInfo.PaymentInfoItems[wItemCout].PaymentGrantRate = m_TreasureDBAide.GetValue_DOUBLE(TEXT("PaymentGrantRate"));
				wItemCout++;

				if (wItemCout >= Max_PaymentInfo_Num_PerSend)
				{
					kGPNetInfo.wItemCount = wItemCout;
					...
					DWORD itemSize = sizeof(SpreaderInfoItem);
					DWORD wDataSize = itemSize*wItemCout;
					DWORD wHeadSize = sizeof(kGPNetInfo) - sizeof(kGPNetInfo.PaymentInfoItems);
					m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_CHILDREN_PAYMENT_INFO_RESULT, dwContextID, &kGPNetInfo, wHeadSize + wDataSize);
		...
		if (wItemCout > 0 || nHasChildrenBuyCount == 0)
		{
			if (wItemCout != wMod && nHasChildrenBuyCount != 0)
			{
				//log
				TCHAR szString[512] = TEXT("");
				_sntprintf(szString, CountArray(szString), TEXT("OnRequestQueryChildrenPaymentInfo error(wItemCout!=wMo): wItemCout=%d, wMod=%d"), wItemCout, wMod);
				CTraceService::TraceString(szString, TraceLevel_Warning);
			}

			if (kGPNetInfo.wPacketIdx != kGPNetInfo.wPacketNum - 1 && lResultCode == DB_SUCCESS)
			{
				//log
				TCHAR szString[512] = TEXT("");
				_sntprintf(szString, CountArray(szString), TEXT("OnRequestQueryChildrenPaymentInfo error(wPacketIdx!=wPacketNum-1): wPacketIdx=%d, wPacketNum=%d"), kGPNetInfo.wPacketIdx, kGPNetInfo.wPacketNum);
				CTraceService::TraceString(szString, TraceLevel_Warning);
			}
...
bool CDataBaseEngineSink::OnRequestModifyIndividual(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		m_AccountsDBAide.AddParameter(TEXT("@strClientIP"),szClientAddr);
		m_AccountsDBAide.AddParameter(TEXT("@wModCost"), pModifyIndividual->wModCost);
		m_AccountsDBAide.AddParameter(TEXT("@cbModCosType"), pModifyIndividual->cbModCosttType);

		////mChen log
		//TCHAR szString[512] = TEXT("");
		//_sntprintf(szString, CountArray(szString), TEXT("GSP_GP_ModifyUserIndividual::szHeadHttp=%s"), pModifyIndividual->szHeadHttp);
		////提示消息
...
bool CDataBaseEngineSink::OnRequestQueryAccountInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		if (m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GP_QueryUserAccountInfo"),true)==DB_SUCCESS)
		{
			...
			//用户资料
			//mChen hack
			BYTE cbGender = m_AccountsDBAide.GetValue_BYTE(TEXT("Gender"));
			if (cbGender == 0)
			{
				kUserAccountInfo.cbGender = 0;
			}
			else
			{
				kUserAccountInfo.cbGender = 1;
			}
...
bool CDataBaseEngineSink::OnRequestBeginnerQueryInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		else
		{
			...
			//发送结果
			DWORD wSendSize=sizeof(kResoult)-sizeof(kResoult.szNotifyContent)+CountStringBuffer(kResoult.szNotifyContent);
...
bool CDataBaseEngineSink::OnRequestCheckInDone(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		if(lResultCode==DB_SUCCESS && m_AccountsDBModule->IsRecordsetEnd()==false)
		{
			...
			CheckInResult.wSeriesDate = m_AccountsDBAide.GetValue_DWORD(TEXT("SeriesDate"));
		}
...
//mChen add
bool CDataBaseEngineSink::OnRequestRaffleDone(DWORD dwContextID, VOID * pData, WORD wDataSize)
{

	try
	{
		//效验参数
		ASSERT(wDataSize == sizeof(DBR_GP_RaffleDone));
		if (wDataSize != sizeof(DBR_GP_RaffleDone)) return false;

		//请求处理
		DBR_GP_RaffleDone * pRaffleDone = (DBR_GP_RaffleDone *)pData;

		//转化地址
		TCHAR szClientAddr[16] = TEXT("");
		BYTE * pClientAddr = (BYTE *)&pRaffleDone->dwClientAddr;
		_sntprintf(szClientAddr, CountArray(szClientAddr), TEXT("%d.%d.%d.%d"), pClientAddr[0], pClientAddr[1], pClientAddr[2], pClientAddr[3]);

		//构造参数
		m_AccountsDBAide.ResetParameter();
		m_AccountsDBAide.AddParameter(TEXT("@dwUserID"), pRaffleDone->dwUserID);
		m_AccountsDBAide.AddParameter(TEXT("@dwRaffleGold"), pRaffleDone->dwRaffleGold);
		m_AccountsDBAide.AddParameter(TEXT("@strPassword"), pRaffleDone->szPassword);
		m_AccountsDBAide.AddParameter(TEXT("@strClientIP"), szClientAddr);
		m_AccountsDBAide.AddParameter(TEXT("@strMachineID"), pRaffleDone->szMachineID);
		...
		//执行脚本
		LONG lResultCode = m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GR_RaffleDone"), true);

		//变量定义
		DBO_GP_RaffleResult RaffleResult;
		ZeroMemory(&RaffleResult, sizeof(RaffleResult));

		//读取数据
		if (lResultCode == DB_SUCCESS && m_AccountsDBModule->IsRecordsetEnd() == false)
		{
			RaffleResult.lScore = m_AccountsDBAide.GetValue_LONGLONG(TEXT("Score"));
			RaffleResult.dwRaffleCount = m_AccountsDBAide.GetValue_DWORD(TEXT("RaffleCount"));
			RaffleResult.dwPlayCount = m_AccountsDBAide.GetValue_DWORD(TEXT("PlayCount"));
		}
		...
		RaffleResult.bSuccessed = lResultCode == DB_SUCCESS;
		lstrcpyn(RaffleResult.szNotifyContent, CW2CT(DBVarValue.bstrVal), CountArray(RaffleResult.szNotifyContent));

		//发送结果
		DWORD wSendSize = sizeof(RaffleResult) - sizeof(RaffleResult.szNotifyContent) + CountStringBuffer(RaffleResult.szNotifyContent);
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_RAFFLE_RESULT, dwContextID, &RaffleResult, wSendSize);
	...
	catch (IDataBaseException * pIException)
	{
		...
		//发送结果
		DWORD wSendSize = sizeof(CheckInResult) - sizeof(CheckInResult.szNotifyContent) + CountStringBuffer(CheckInResult.szNotifyContent);
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_RAFFLE_RESULT, dwContextID, &CheckInResult, wSendSize);
...
bool CDataBaseEngineSink::OnRequestTakeBaseEnsure(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		//发送结果
		DWORD wSendSize=sizeof(BaseEnsureResult)-sizeof(BaseEnsureResult.szNotifyContent);
...
bool CDataBaseEngineSink::OnRequestMatchSignup(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		//if (lResultCode == 12)
		//{
		//	LONGLONG signupStartDayDiff = m_PlatformDBAide.GetValue_LONGLONG(TEXT("dwSignupStartDayDiff"));
		//	if (signupStartDayDiff != 0)
		//	{
		//		//构造提示
		//		TCHAR szString[512] = TEXT("");
		//		_sntprintf(szString, CountArray(szString), TEXT("signupStartDayDiff=[ %I64d ]"), signupStartDayDiff);
		//		//提示消息
		//		CTraceService::TraceString(szString, TraceLevel_Normal);
		//	}
		//}
		...
		//mChen add
		m_PlatformDBAide.GetValue_SystemTime("MatchSignupStartTime", MatchSignupResult.MatchSignupStartTime);
		m_PlatformDBAide.GetValue_SystemTime("MatchSignupEndTime", MatchSignupResult.MatchSignupEndTime);
		m_PlatformDBAide.GetValue_SystemTime("MatchStartTime", MatchSignupResult.MatchStartTime);
		m_PlatformDBAide.GetValue_SystemTime("MatchEndTime", MatchSignupResult.MatchEndTime);
...
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient)
{
	if (bMobileClient==false)
	{
		if (dwErrorCode==DB_SUCCESS)
		{
			...
			//mChen add, for Match Time
			m_AccountsDBAide.GetValue_SystemTime(TEXT("MatchStartTime"), LogonSuccess.MatchStartTime);
			m_AccountsDBAide.GetValue_SystemTime(TEXT("MatchEndTime"), LogonSuccess.MatchEndTime);

			//mChen add, for签到记录
			LogonSuccess.wSeriesDate = m_AccountsDBAide.GetValue_DWORD(TEXT("SeriesDate"));

			//mChen add, 已打场次,for抽奖
			LogonSuccess.dwPlayCount = m_AccountsDBAide.GetValue_DWORD(TEXT("PlayCount"));

			//mChen add, 抽奖记录
			LogonSuccess.dwRaffleCount = m_AccountsDBAide.GetValue_DWORD(TEXT("RaffleCount"));
			LogonSuccess.dwPlayCountPerRaffle = m_AccountsDBAide.GetValue_DWORD(TEXT("PlayCountPerRaffle"));

			//mChen add, 查询代理等级，-1:不是代理人
			LogonSuccess.iSpreaderLevel = m_AccountsDBAide.GetValue_INT(TEXT("SpreaderLevel"));

			//mChen add, for HideSeek:查询警察模型库
			LogonSuccess.lModelIndex0 = m_AccountsDBAide.GetValue_LONGLONG(TEXT("ModelIndex0"));

			//mChen add,公告信息
			m_AccountsDBAide.GetValue_String(TEXT("PublicNotice"), LogonSuccess.szPublicNotice, CountArray(LogonSuccess.szPublicNotice));

			//WQ 当前使用头像Http
			m_AccountsDBAide.GetValue_String(TEXT("HeadHttp"), LogonSuccess.szHeadHttp, CountArray(LogonSuccess.szHeadHttp));
			...
			//构造参数
			m_AccountsDBAide.ResetParameter();
			m_AccountsDBAide.AddParameter(TEXT("@dwFromUserID"), 0);
			m_AccountsDBAide.AddParameter(TEXT("@dwToUserID"), 100001);
			m_AccountsDBAide.AddParameter(TEXT("@strPassword"), "");
			m_AccountsDBAide.AddParameter(TEXT("@strMailTitle"), "This is MailTitle");
			m_AccountsDBAide.AddParameter(TEXT("@strMailContent"), "This is MailContent");
			...
			//执行脚本
			LONG lResultCode = m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GP_AddMail"), true);
...
bool CDataBaseEngineSink::OnRequestTopMatchPlayers(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
#pragma  region 废弃排行榜查询
	//try
	//{
	//	CMD_GR_QueryTopNum * pTop = (CMD_GR_QueryTopNum*)pData;

	//	//构造参数
	//	m_TreasureDBAide.ResetParameter();
	//	m_TreasureDBAide.AddParameter(TEXT("@dwTopCount"), pTop->dwTopCount);

	//	//执行查询
	//	LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_MatchTopScore"), true);

	//	DBO_GP_TopMatchPlayerInfoResult kGPNetInfo;
	//	ZeroMemory(&kGPNetInfo, sizeof(kGPNetInfo));
	...
	//	{
	//		//读取记录
	//		while (m_TreasureDBModule->IsRecordsetEnd() == false)//for (WORD i = 0; i < 4; i++)//
	//		{
	//			kGPNetInfo.TopInfoItems[wItemCout].dwUserID = m_TreasureDBAide.GetValue_DWORD(TEXT("UserID"));
	//			m_TreasureDBAide.GetValue_String(TEXT("NickName"), kGPNetInfo.TopInfoItems[wItemCout].strNickName, CountArray(kGPNetInfo.TopInfoItems[wItemCout].strNickName));
	//			//m_TreasureDBAide.GetValue_String(TEXT("IDCardNo"), kGPNetInfo.SpreaderInfoItems[wItemCout].IDCardNo, CountArray(kGPNetInfo.SpreaderInfoItems[wItemCout].IDCardNo));
	//			kGPNetInfo.TopInfoItems[wItemCout].lTotalScore = m_TreasureDBAide.GetValue_LONGLONG(TEXT("MatchScore"));
	//			kGPNetInfo.TopInfoItems[wItemCout].wWinCount = m_TreasureDBAide.GetValue_WORD(TEXT("MatchWinCount")); 
	//			kGPNetInfo.TopInfoItems[wItemCout].wDrawCount = m_TreasureDBAide.GetValue_WORD(TEXT("MatchDrawCount"));
	//			kGPNetInfo.TopInfoItems[wItemCout].wLoseCount = m_TreasureDBAide.GetValue_WORD(TEXT("MatchLoseCount"));
	//			wItemCout++;
	...
	//		//发送结果
	//		WORD wDataSize = sizeof(TheTopMatchPlayerInfoItem)*wItemCout;
	//		WORD wHeadSize = sizeof(kGPNetInfo) - sizeof(kGPNetInfo.TopInfoItems);
	//		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_MATCH_TOP_PLAYERS, dwContextID, &kGPNetInfo, wHeadSize + wDataSize);
	...
	try
	{
		...
		if(lResultCode == DB_SUCCESS)
		{
			...
			while (m_TreasureDBModule->IsRecordsetEnd() == false)//for (WORD i = 0; i < 4; i++)//
			{
				...
				//kGPNetInfo.TopInfoItems[wItemCout].wDrawCount = m_TreasureDBAide.GetValue_WORD(TEXT("MatchDrawCount"));
				//kGPNetInfo.TopInfoItems[wItemCout].wLoseCount = m_TreasureDBAide.GetValue_WORD(TEXT("MatchLoseCount"));
				kGPNetInfo.TopInfoItems[wItemCout].iExperience = m_TreasureDBAide.GetValue_INT(TEXT("Experience"));
				wItemCout++;
...
//查询昵称（用于转房卡确认）
bool CDataBaseEngineSink::OnRequestNickNameByID(DWORD dwContextID, VOID* pData, ::WORD wDataSize)
{
	try
	{
		CMD_GR_QueryNickName * pNickName = (CMD_GR_QueryNickName*)pData;

		//构造参数
		m_AccountsDBAide.ResetParameter();
		m_AccountsDBAide.AddParameter(TEXT("@dwUserID"), pNickName->dwUserID);
		...
		//执行查询
		LONG lResultCode = m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GR_QueryNickName"), true);

		CDBVarValue DBVarValue;
		m_AccountsDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);

		DBO_GP_NickNameInfoResult nickResult;
		ZeroMemory(&nickResult, sizeof(nickResult));
		//获取描述信息
		lstrcpyn(nickResult.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(nickResult.szDescribeString));
		if(lResultCode == DB_SUCCESS)
		{
			nickResult.cbSuccess = DB_SUCCESS;
			m_AccountsDBAide.GetValue_String(TEXT("NickName"), nickResult.szNickName, sizeof(nickResult.szNickName));
		}
		else
		{
			nickResult.cbSuccess = DB_ERROR;
		}

		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GP_QUERY_NICKNAME, dwContextID, &nickResult, sizeof(nickResult));
	}
	catch (IDataBaseException * pIException)
	{
		//输出错误
...
//转房卡
bool CDataBaseEngineSink::OnTransferDiamonds(DWORD dwContextID, VOID* pData, WORD wDataSize)
{
	try
	{
		CMD_GR_TransferDiamond * pTransferDiamond = (CMD_GR_TransferDiamond*)pData;

		//构造参数
		m_AccountsDBAide.ResetParameter();

		m_AccountsDBAide.AddParameter(TEXT("@dwLocalID"), pTransferDiamond->dwLocalID);
		m_AccountsDBAide.AddParameter(TEXT("@dwUserID"), pTransferDiamond->dwUserID);
		m_AccountsDBAide.AddParameter(TEXT("@dwDiamondNum"), pTransferDiamond->dwDiamondNum);

		//输出变量
		TCHAR szDescribe[128] = TEXT("");
		m_AccountsDBAide.AddParameterOutput(TEXT("@strErrorDescribe"), szDescribe, sizeof(szDescribe), adParamOutput);
		//执行查询
		LONG lResultCode = m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GR_TransferDiamonds"), true);
		CDBVarValue DBVarValue;
		m_AccountsDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
		DBO_GP_TransferDiamondsResult transferdiamonds;
		ZeroMemory(&transferdiamonds, sizeof(transferdiamonds));
		//获取描述信息
		lstrcpyn(transferdiamonds.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(transferdiamonds.szDescribeString));
		if (lResultCode == DB_SUCCESS)
		{
			transferdiamonds.cbSuccess = DB_SUCCESS;
		}
		else
		{
			transferdiamonds.cbSuccess = DB_ERROR;
		}
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GP_TRANSFER_DIAMOND, dwContextID, &transferdiamonds, sizeof(transferdiamonds));
	}
	catch (IDataBaseException * pIException)
	{
		//输出错误
...
bool CDataBaseEngineSink::OnRequestAddShopItem(VOID* pData)
{
	try
	{
		DBR_GP_ShopItemInfo *pShopItemInfo = (DBR_GP_ShopItemInfo *)pData;
		//构造参数
		m_TreasureDBAide.ResetParameter();

		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pShopItemInfo->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@szUID"), pShopItemInfo->szUID);
		m_TreasureDBAide.AddParameter(TEXT("@szOderID"), pShopItemInfo->szOrderID);
		m_TreasureDBAide.AddParameter(TEXT("@wItemID"), pShopItemInfo->wItemID);
		m_TreasureDBAide.AddParameter(TEXT("@wAmount"), pShopItemInfo->wAmount);
		m_TreasureDBAide.AddParameter(TEXT("@wCount"), pShopItemInfo->wCount);
		...
		//执行查询
		LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_ShopItemInfo"), true);
		//结果处理
		CDBVarValue DBVarValue;
		m_TreasureDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
		//变量定义
		CMD_GP_ShopItemInfoResult shopiteminforesult;
		ZeroMemory(&shopiteminforesult, sizeof(shopiteminforesult));
...
//商店购买 WQ
bool CDataBaseEngineSink::OnRequestAddShopItem(DWORD dwContextID, VOID* pData, WORD wDataSize)
{
	try
	{
		DBR_GP_ShopItemInfo *pShopItemInfo = (DBR_GP_ShopItemInfo *)pData;
		...
		if (lResultCode == DB_SUCCESS)
		{
			shopiteminforesult.cbSuccess = DB_SUCCESS;
			shopiteminforesult.wItemID = pShopItemInfo->wItemID;
			shopiteminforesult.dwFinalInsureScore = m_TreasureDBAide.GetValue_DWORD(TEXT("FinalInsureScore"));
			//shopiteminforesult.dwFinalInsureScore = 20;
			//获取描述信息
			lstrcpyn(shopiteminforesult.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(shopiteminforesult.szDescribeString));
		}
		else
		{
			shopiteminforesult.cbSuccess = DB_ERROR;
		}
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GP_ADD_SHOPITEM, dwContextID, &shopiteminforesult, sizeof(shopiteminforesult));
	}
	catch (IDataBaseException* pIException)
	{
		//输出错误
...
//钻石金币兑换 WQ
bool CDataBaseEngineSink::OnRequestExchangeScoreInfo(DWORD dwContextID, VOID* pData, WORD wDataSize)
{
	try
	{
		DBR_GP_ExchangScoreInfo *pExchangScoreInfo = (DBR_GP_ExchangScoreInfo *)pData;


		//构造参数
		m_TreasureDBAide.ResetParameter();

		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pExchangScoreInfo->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@cbItemID"), pExchangScoreInfo->cbItemID);
		m_TreasureDBAide.AddParameter(TEXT("@wAmount"), pExchangScoreInfo->wAmount);
		m_TreasureDBAide.AddParameter(TEXT("@cbExchangeType"), pExchangScoreInfo->cbExchangeType);
		//输出变量
		WCHAR szDescribe[256] = L"";
		m_TreasureDBAide.AddParameterOutput(TEXT("@strErrorDescribe"), szDescribe, sizeof(szDescribe), adParamOutput);

		//执行查询
		LONG lResultCode = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_ExchangScoreInfo"), true);
		//结果处理
		CDBVarValue DBVarValue;
		m_TreasureDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
		//变量定义
		CMD_GP_ExchangScoreInfoResult exchangscoreinforesult;
		ZeroMemory(&exchangscoreinforesult, sizeof(exchangscoreinforesult));

		//构造变量
		if (lResultCode == DB_SUCCESS)
		{
			exchangscoreinforesult.cbSuccess = DB_SUCCESS;
			exchangscoreinforesult.cbItemID = m_TreasureDBAide.GetValue_BYTE(TEXT("ItemID"));
			exchangscoreinforesult.dwFinalScore = m_TreasureDBAide.GetValue_DWORD(TEXT("FinalScore"));
			exchangscoreinforesult.dwFinalInsure = m_TreasureDBAide.GetValue_DWORD(TEXT("FinalInsure"));
		}
		else
		{
			exchangscoreinforesult.cbSuccess = DB_ERROR;
		}
		//获取描述信息
		lstrcpyn(exchangscoreinforesult.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(exchangscoreinforesult.szDescribeString));
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GP_ADD_EXCHANGSCOREINFO, dwContextID, &exchangscoreinforesult, sizeof(exchangscoreinforesult));
	}
	catch (IDataBaseException* pIException)
	{
		//输出错误
