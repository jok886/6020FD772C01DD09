...
bool CPrivateServiceManager::StopService()
{
	...
	{
		m_pIGamePrivatetem->OnStopService();
		SafeRelease(m_pIGamePrivatetem);
	}
...
bool CPrivateServiceManager::OnEventReqStandUP(IServerUserItem * pIServerUserItem, BYTE cbForceStandUP)
{
	ASSERT(m_pIGamePrivatetem!=NULL);
	if(m_pIGamePrivatetem!=NULL)
	{
		return m_pIGamePrivatetem->OnEventReqStandUP(pIServerUserItem, cbForceStandUP);
	}
	return true;
}
