...
#define TIME_BREAK_CONNECT			5000L								//中断时间
...
bool CServerSocketItem::SendData(WORD wMainCmdID, WORD wSubCmdID, WORD wRountID)
{
	...
	if (m_wRountID != wRountID)
	{
		...
		_sntprintf(szString, CountArray(szString), TEXT("CServerSocketItem::SendData：m_wRountID!=wRountID, m_wRountID=%d， wRountID=%d"), m_wRountID, wRountID);
	...
	DWORD wSendSize = EncryptBuffer(pOverLappedSend->m_cbBuffer, sizeof(TCP_Head), sizeof(pOverLappedSend->m_cbBuffer));
...
//发送函数
bool CServerSocketItem::SendData(void * pData, DWORD wDataSize, WORD wMainCmdID, WORD wSubCmdID, WORD wRountID)
{
	//效验参数
	...
	if (m_dwRecvPacketCount == 0)
	{
		// Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("CServerSocketItem::SendData：m_dwRecvPacketCount==0"));
		CTraceService::TraceString(szString, TraceLevel_Warning);
	...
	if (IsValidSocket() == false)
	{
		// Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("CServerSocketItem::SendData：IsValidSocket() == false"));
		CTraceService::TraceString(szString, TraceLevel_Warning);
	...
	if (wDataSize > SOCKET_TCP_BUFFER)
	{
		// Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("CServerSocketItem::SendData：wDataSize > SOCKET_TCP_BUFFER, wDataSize=%d"), wDataSize);
		CTraceService::TraceString(szString, TraceLevel_Warning);
	...
	DWORD wSendSize = EncryptBuffer(pOverLappedSend->m_cbBuffer, sizeof(TCP_Head) + wDataSize, sizeof(pOverLappedSend->m_cbBuffer));
	pOverLappedSend->m_WSABuffer.len = wSendSize;
...
bool CServerSocketItem::RecvData()
{
	...
	if (m_bCloseIng == true)
	{
		...
		{
			CloseSocket(m_wRountID,"CServerSocketItem::RecvData:m_OverLappedSendActive.GetCount() == 0");
		}
		return false;
	}
	...
	{
		m_bRecvIng = false;
		CloseSocket(m_wRountID, __FUNCTION__);
		return false;
	}
...
//关闭连接
bool CServerSocketItem::CloseSocket(WORD wRountID, char *pFunc)
{
	//mChen add,Log
	TCHAR szString[512] = TEXT("");
	_sntprintf(szString, CountArray(szString), TEXT("CServerSocketItem::CloseSocket: m_wRountID=%d, wRountID=%d, pFunc = %s"), m_wRountID, wRountID, pFunc);
	CTraceService::TraceString(szString, TraceLevel_Warning);
...
bool CServerSocketItem::OnSendCompleted(COverLappedSend * pOverLappedSend, DWORD dwThancferred)
{
	...
	if (m_hSocket == INVALID_SOCKET)
	{
		m_OverLappedSendFree.Append(m_OverLappedSendActive);
		m_OverLappedSendActive.RemoveAll();
		CloseSocket(m_wRountID, "CServerSocketItem::OnSendCompleted:m_hSocket == INVALID_SOCKET");
		return true;
	}
	...
	if (m_OverLappedSendActive.GetCount() > 0)
	{
		...
		if ((iRetCode == SOCKET_ERROR) && (WSAGetLastError() != WSA_IO_PENDING))
		{
			m_OverLappedSendFree.Append(m_OverLappedSendActive);
			m_OverLappedSendActive.RemoveAll();
			CloseSocket(m_wRountID, "CServerSocketItem::OnSendCompleted:iRetCode == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING");
			return false;
		}
		return true;
	}
	...
	//判断关闭
	if (m_bCloseIng == true)
		CloseSocket(m_wRountID, "CServerSocketItem::OnSendCompleted:m_bCloseIng == tru");
...
bool CServerSocketItem::OnRecvCompleted(COverLappedRecv * pOverLappedRecv, DWORD dwThancferred)
{
	...
	if (m_hSocket == INVALID_SOCKET)
	{
		CloseSocket(m_wRountID,"OnRecvCompleted:m_hSocket == INVALID_SOCKET");
		return true;
	}
	...
	if (iRetCode <= 0)
	{
		CloseSocket(m_wRountID, "OnRecvCompleted:iRetCode <= 0");
		return true;
	}
	...
	try
	{
		...
		{
			//效验数据
			DWORD wPacketSize = pHead->TCPInfo.wPacketSize;
			...
			DWORD wRealySize = CrevasseBuffer(cbBuffer, wPacketSize);
	...
	catch (LPCTSTR pszError)
	{
		...
		_sntprintf(szString, CountArray(szString), TEXT("OnRecvCompleted:catch, pszError=%s"), pszError);

		CloseSocket(m_wRountID, szString);
		return false;
	}
...
//加密数据
DWORD CServerSocketItem::EncryptBuffer(BYTE pcbDataBuffer[], DWORD wDataSize, DWORD wBufferSize)
{
	DWORD i = 0;
	//效验参数
	...
	//调整长度
	DWORD wEncryptSize = wDataSize - sizeof(TCP_Info), wSnapCount = 0;
	...
	if (wDataSize > 1000)
	{
		// Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("CServerSocketItem::EncryptBuffer：wDataSize=%d, cbDataKind=%d, wMainCmdID=%d, wSubCmdID=%d"), wDataSize, pHead->TCPInfo.cbDataKind, pHead->CommandInfo.wMainCmdID, pHead->CommandInfo.wSubCmdID);
		CTraceService::TraceString(szString, TraceLevel_Warning);
	}
...
//解密数据
DWORD CServerSocketItem::CrevasseBuffer(BYTE pcbDataBuffer[], DWORD wDataSize)
{
	DWORD i = 0;
	//效验参数
	ASSERT(wDataSize >= sizeof(TCP_Head));
	...
	if (cbCheckCode != 0)
	{
		//mChen add,Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("CServerSocketItem::CrevasseBuffer:数据包效验码错误:wMainCmdID=%d,wSubCmdID=%d, wPacketSize=%d"), pHead->CommandInfo.wMainCmdID, pHead->CommandInfo.wSubCmdID, pHead->TCPInfo.wPacketSize);
		CTraceService::TraceString(szString, TraceLevel_Warning);

		//mChen comment, hack断线 temp
		///throw TEXT("数据包效验码错误");
	}
...
bool  CTCPNetworkEngine::StartService()
{
	...
	try
	{
		...
		if (bSuccess == false) throw TEXT("网络引擎网络监听线程服务配置失败");
...
bool  CTCPNetworkEngine::ConcludeService()
{
	...
	for (i = 0; i < m_ActiveSocketItem.GetCount(); i++)
	{
		...
		ASSERT(pServerSocketItem != NULL);
		pServerSocketItem->CloseSocket(pServerSocketItem->GetRountID(),"ConcludeService");
			...
			for (INT_PTR i = 0; i < m_TempSocketItem.GetCount(); i++)
			{
				...
				if (pServerSocketItem->IsReadySend() == true)
				{
					if ((dwNowTickCount - dwRecvTickCount) > dwBreakTickCount)
					{
						//mChen add,Log
						TCHAR szString[128] = TEXT("");
						_sntprintf(szString, CountArray(szString), TEXT("QUEUE_DETECT_SOCKET:IsReadySend true : CloseSocket when TickCount > dwBreakTickCount, RecvPacketCoun=%d"), pServerSocketItem->GetRecvPacketCount());
						CTraceService::TraceString(szString, TraceLevel_Warning);

						pServerSocketItem->CloseSocket(pServerSocketItem->GetRountID(),"TickCount > dwBreakTickCoun");
						continue;
					}
				}
				else
				{
					...
					{
						//mChen add,Log
						CTraceService::TraceString(TEXT("QUEUE_DETECT_SOCKET:IsReadySend false: CloseSocket when TickCount > TIME_BREAK_CONNECT "), TraceLevel_Warning);

						pServerSocketItem->CloseSocket(pServerSocketItem->GetRountID(),"TickCount > TIME_BREAK_CONNEC");
						continue;
					}
				}
				...
				{
					//Log:提示消息
					CTraceService::TraceString(TEXT("QUEUE_DETECT_SOCKET:IsReadySend==false"), TraceLevel_Normal);
				}
			}
...
bool  CTCPNetworkEngine::SendData(DWORD dwSocketID, WORD wMainCmdID, WORD wSubCmdID)
{
	...
	//加入发送队列
	DWORD wSendSize = sizeof(SendRequest) - sizeof(SendRequest.cbSendBuf);
	return m_SendQueueService.AddToQueue(QUEUE_SEND_REQUEST, &SendRequest, wSendSize);
}

//发送函数
bool  CTCPNetworkEngine::SendData(DWORD dwSocketID, WORD wMainCmdID, WORD wSubCmdID, void * pData, DWORD wDataSize)
{
	//效益状态
	ASSERT(m_bService == true);
	if (m_bService == false) return false;
	...
	//加入发送队列
	DWORD wSendSize = sizeof(SendRequest) - sizeof(SendRequest.cbSendBuf) + wDataSize;
	if (wSendSize > 1000)
	{
		// Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("CTCPNetworkEngine::SendData：wSendSize=%d, wMainCmdID=%d, wSubCmdID=%d"), wSendSize, wMainCmdID, wSubCmdID);
		CTraceService::TraceString(szString, TraceLevel_Warning);
	}
	return m_SendQueueService.AddToQueue(QUEUE_SEND_REQUEST, &SendRequest, wSendSize);
}
...
//批量发送
bool  CTCPNetworkEngine::SendDataBatch(WORD wMainCmdID, WORD wSubCmdID, void * pData, DWORD wDataSize,LONG)
{
	//效益状态
	if (m_bService == false) return false;
	...
	if (wSendSize > 1000)
	{
		// Log
		TCHAR szString[128] = TEXT("");
		_sntprintf(szString, CountArray(szString), TEXT("CTCPNetworkEngine::SendDataBatch：wSendSize=%d, wMainCmdID=%d, wSubCmdID=%d"), wSendSize, wMainCmdID, wSubCmdID);
		CTraceService::TraceString(szString, TraceLevel_Warning);
	}
	return m_SendQueueService.AddToQueue(QUEUE_SEND_REQUEST, &SendRequest, wSendSize);
}
...
//关闭连接
bool  CTCPNetworkEngine::CloseSocket(DWORD dwSocketID, char *pFunc)
{
	CServerSocketItem * pServerSocketItem = EnumSocketItem(LOWORD(dwSocketID));
	if (pServerSocketItem == NULL) return false;
	CWHDataLocker lock(pServerSocketItem->GetSignedLock());
	return pServerSocketItem->CloseSocket(HIWORD(dwSocketID), pFunc);
}
