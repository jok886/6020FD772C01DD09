...
public:
	...
	//发送函数
	bool SendData(void * pData, DWORD wDataSize, WORD wMainCmdID, WORD wSubCmdID, WORD wRountID);
	//接收操作
	bool RecvData();
	//关闭连接
	bool CloseSocket(WORD wRountID, char *pFunc);
	//设置关闭
...
private:
	//加密数据
	DWORD EncryptBuffer(BYTE pcbDataBuffer[], DWORD wDataSize, DWORD wBufferSize);
	//解密数据
	DWORD CrevasseBuffer(BYTE pcbDataBuffer[], DWORD wDataSize);
...
public:
	...
	//批量发送
	virtual bool  SendDataBatch(WORD wMainCmdID, WORD wSubCmdID, void * pData, DWORD wDataSize,LONG);
	//关闭连接
	virtual bool  CloseSocket(DWORD dwSocketID, char *pFunc);
	//设置关闭
