...
//插入请求
bool  CAsynchronismEngine::InsertRequest(WORD wRequestID, void * const pBuffer, DWORD wDataSize, IUnknownEx * pIUnknownEx)
{
	//效验状态
...
//投递数据
bool  CAsynchronismEngine::PostAsynchronismData(WORD wIdentifier, VOID * pData, DWORD wDataSize)
{
	//效验状态
	ASSERT(wDataSize <= sizeof(m_cbBuffer));
	if (m_bService == false) return false;
