...
//加入数据
bool  CQueueService::AddToQueue(WORD wIdentifier, void * const pBuffer, DWORD wDataSize)//mChen edit WORD, fix size bug
{
	CWHDataLocker lock(m_CriticalSection);//
...
//提取数据
bool CQueueService::GetData(tagDataHead & DataHead, void * pBuffer, DWORD wBufferSize)//mChen edit WORD, fix size bug
{
	CWHDataLocker lock(m_CriticalSection);//
...
//数据消息
void CQueueService::OnQueueServiceThread(const tagDataHead & DataHead, void * pBuffer, DWORD wDataSize)//mChen edit WORD, fix size bug
{
	ASSERT(m_pIQueueServiceSink != NULL);
	try
	{
