	...
	switch (wIdentifier)
	{
		...
		case EVENT_TCP_NETWORK_ACCEPT:	//网络应答事件
		{
			...
			if (bSuccess == false)
			{
				...
				_sntprintf(szString, CountArray(szString), TEXT("EVENT_TCP_NETWORK_ACCEPT: OnEventTCPNetworkBind result == false"));
				CTraceService::TraceString(szString, TraceLevel_Normal);

				//mChen comment, hack断线 temp
				//m_pTCPNetworkEngine->CloseSocket(pSocketAcceptEvent->dwSocketID, "CAttemperEngine::OnQueueServiceSink:EVENT_TCP_NETWORK_ACCEPT, bSuccess == false");
			}
		...
		case EVENT_TCP_NETWORK_READ:	//网络读取事件
		{
			...
			if (bSuccess == false)
			{
				//log
				TCHAR kTemp[256];
				sprintf_s(kTemp, "EVENT_TCP_NETWORK_READ: OnEventTCPNetworkRead result == false, wMainCmdID=%d, wSubCmdID=%d", pSocketReadEvent->Command.wMainCmdID, pSocketReadEvent->Command.wSubCmdID);
				CTraceService::TraceString(kTemp, TraceLevel_Exception);

				//mChen comment, hack断线 temp
				//m_pTCPNetworkEngine->CloseSocket(pSocketReadEvent->dwSocketID, "CAttemperEngine::OnQueueServiceSink:EVENT_TCP_NETWORK_READ:bSuccess == false");
		...
		case EVENT_TCP_NETWORK_CLOSE:	//关闭读取事件
		{
			...
			if (bSuccess == false)
			{
				m_pTCPNetworkEngine->CloseSocket(pSocketCloseEvent->dwSocketID,"EVENT_TCP_NETWORK_CLOSE:bSuccess == false");
			}
...
bool  CAttemperEngine::OnEventDataBaseResult(WORD wRequestID, DWORD dwContextID, VOID * pData, DWORD wDataSize)
{
	try
	{
