...
#define MAX_ASYNCHRONISM_DATA		102400								//异步数据	mChen edit 20480, fix size bug
...
interface IAsynchronismEngine : public IServiceModule
{
	...
	//投递数据
	virtual bool  PostAsynchronismData(WORD wIdentifier, VOID * pData, DWORD wDataSize)=NULL;
};
...
public:
	...
	//发送函数
	virtual bool  SendData(DWORD dwSocketID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
	//批量发送
	virtual bool  SendDataBatch(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize,LONG)=NULL;

	//控制接口
public:
	//关闭连接
	virtual bool  CloseSocket(DWORD dwSocketID, char *pFunc)=NULL;
	//设置关闭
...
public:
	...
	//发送函数
	virtual bool  SendData(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
};
...
public:
	//数据库结果
	virtual bool  OnEventDataBaseResult(WORD wRequestID, DWORD dwContextID, VOID * pData, DWORD wDataSize)=NULL;
};
...
public:
	...
	//数据库事件
	virtual bool  OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, DWORD wDataSize)=NULL;
