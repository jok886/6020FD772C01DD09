...
interface IQueueService : public IServiceModule
{
	...
	//加入数据
	virtual bool  AddToQueue(WORD wIdentifier, void * const pBuffer, DWORD wDataSize) = NULL;//mChen edit WORD, fix size bug
};
...
public:
	//加入数据
	virtual bool  AddToQueue(WORD wIdentifier, void * const pBuffer, DWORD wDataSize);
	//设置接口
...
private:
	//提取数据
	bool GetData(tagDataHead & DataHead, void * pBuffer, DWORD wBufferSize);
	//数据消息
	void OnQueueServiceThread(const tagDataHead & DataHead, void * pBuffer, DWORD wDataSize);
};
