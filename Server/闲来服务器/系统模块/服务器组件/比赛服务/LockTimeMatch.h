...
//时钟定义
#define IDI_CHECK_START_SIGNUP		(IDI_MATCH_MODULE_START+1)					//报名开始时钟 //mChen add
...
{
	MatchStatus_Free=0,
	MatchStatus_Signingup,//mChen add
	MatchStatus_Playing,
	MatchStatus_End
};
...
protected:
	...
	//mChen add
private:
	WORD m_wCurRound;//当前轮数
	WORD m_wFinishTable;//完成一轮的桌数
	DWORD*	m_pITablePlayCount;				//每桌已完场数
...
protected:
	...
	//mChen add
	void SelectPromotionUserOfTable(ITableFrame *pITableFrame);
	void CreateMatchCost(ITableFrame* pITableFrame);
...
protected:
	...
	DWORD GetSignupStartInterval();//mChen add:报名开始间隔
	//开始间隔
	DWORD GetMatchStartInterval();	
	DWORD GetMatchEndInterval();//mChen add:结束间隔
	//获得名次
