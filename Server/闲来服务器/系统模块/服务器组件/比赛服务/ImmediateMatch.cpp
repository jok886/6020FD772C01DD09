...
bool CImmediateMatch::OnEventDataBase(WORD wRequestID, IServerUserItem * pIServerUserItem, VOID * pData, WORD wDataSize)
{
	...
	{
		//mChen add
	case DBO_GR_MATCH_SIGNUP_CHECK_RESULT:		//报名查询结果
	{
		//参数效验
		if (pIServerUserItem == NULL) return true;
		if (wDataSize > sizeof(DBO_GR_MatchSingupCheckResult)) return false;

		//提取数据
		DBO_GR_MatchSingupCheckResult * pMatchSignupCheckResult = (DBO_GR_MatchSingupCheckResult*)pData;

		if (pMatchSignupCheckResult->bInGamePhase)
		{
			//比赛阶段

			if (pMatchSignupCheckResult->bResultCode == false)
			{
				//还未报名

				SendRoomMessage(pIServerUserItem, TEXT("您还未报名，请先报名再来参加比赛！"), SMT_CHAT | SMT_EJECT);
				return true;
			}
			else
			{
				...
				WORD wMessageType;
				if (VerifyMatchTime(szDescribe, CountArray(szDescribe), wMessageType, true) == false)
				{
					//比赛已经开始
				...
				for (INT_PTR i = 0; i < dwUserCount; i++)
				{
					...
					{
						SendRoomMessage(pIServerUserItem, TEXT("您已经参赛，请等待游戏开始！"), SMT_CHAT | SMT_EJECT);
						return true;
					}
				}
				...
				{
					SendRoomMessage(pIServerUserItem, TEXT("参赛失败"), SMT_CHAT);
					return false;
				}
				...
				SendRoomMessage(pIServerUserItem, TEXT("加入比赛成功，请等待游戏开始！"), SMT_CHAT | SMT_EJECT);
			}
		}
		else
		{
	...
	case DBO_GR_MATCH_SIGNUP_RESULT:		//报名结果
		{
			...
			{
				//mChen
				CTime MatchStartTime(m_pMatchOption->MatchType.Immediate.MatchStartTime);
				TCHAR szMsg[256] = TEXT("");
				_sntprintf(szMsg, CountArray(szMsg), TEXT("恭喜，报名成功,请您于%d年%d月%d日-%d时%d分%d秒来参加比赛！"), MatchStartTime.GetYear(), MatchStartTime.GetMonth(), MatchStartTime.GetDay(), MatchStartTime.GetHour(), MatchStartTime.GetMinute(), MatchStartTime.GetSecond());
				SendRoomMessage(pIServerUserItem, szMsg, SMT_CHAT);
...
bool CImmediateMatch::OnEventSocketMatch(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	...
	{
		//mChen add
	case SUB_GR_MATCH_SIGNUP_CHECK:
	{
		//通过查询报名（费用）数据库来判断是否已经报名
		...
		//提取数据
		bool bInGamePhase = *(bool*)pData;

		//变量定义
		DBR_GR_MatchSignupCheck MatchSignupCheck;
		ZeroMemory(&MatchSignupCheck, sizeof(MatchSignupCheck));
		//构造结构
		MatchSignupCheck.dwUserID = pIServerUserItem->GetUserID();
		MatchSignupCheck.dwClientAddr = pIServerUserItem->GetClientAddr();
		MatchSignupCheck.dwMatchID = m_pMatchOption->dwMatchID;
		MatchSignupCheck.dwMatchNO = m_pCurMatchGroup->m_dwMatchNO;//m_pMatchOption->dwMatchNO;// mChen
		MatchSignupCheck.bInGamePhase = bInGamePhase;
		lstrcpyn(MatchSignupCheck.szMachineID, pIServerUserItem->GetMachineID(), CountArray(MatchSignupCheck.szMachineID));
		//投递数据
		m_pIDataBaseEngine->PostDataBaseRequest(MatchSignupCheck.dwUserID, DBR_GR_MATCH_SIGNUP_CHECK, dwSocketID, &MatchSignupCheck, sizeof(MatchSignupCheck));
...
bool CImmediateMatch::OnEventUserJoinMatch(IServerUserItem * pIServerUserItem, BYTE cbReason,DWORD dwSocketID)
{
	...
	//mChen add 1
	//报名阶段已经移到公共报名
	//已经报过名，直接参赛（以下为从DBO_GR_MATCH_SIGNUP_RESULT搬过来）
	//加入比赛
	if(m_pCurMatchGroup->OnUserSignUpMatch(pIServerUserItem)==false)
	{
		SendRoomMessage(pIServerUserItem, TEXT("加入比赛失败"),SMT_CHAT);
		return false;
	}
	//加入比赛成功
	m_OnMatchUserItem.Add(pIServerUserItem);
	SendMatchInfo(NULL);
	SendRoomMessage(pIServerUserItem, TEXT("加入比赛成功，系统正在努力为您分配座位，请稍后。。。"), SMT_CHAT | SMT_EJECT);
	return true;

	//mChen add 2
	//检测是否已经报名:通过查询报名（费用）数据库来判断
	bool bInGamePhase = true;
	OnEventSocketMatch(SUB_GR_MATCH_SIGNUP_CHECK, &bInGamePhase, sizeof(bInGamePhase), pIServerUserItem, dwSocketID);
	return true;
	...
	//mChen待做：放到OnEventDataBase数据库事件中处理
	//真人扣报名费
...
bool CImmediateMatch::OnEventUserQuitMatch(IServerUserItem * pIServerUserItem, BYTE cbReason, WORD *pBestRank, DWORD dwContextID)
{
	...
	if(RemoveMatchUserItem(pIServerUserItem))
	{
		...
		if(m_pCurMatchGroup->OnUserQuitMatch(pIServerUserItem))
		{
			//mChen add
			return true;
			if (m_pCurMatchGroup->m_enMatchStatus > MatchStatus_Signup)
			{
				return true;
			}
...
//mChen add
//事件校验
bool CImmediateMatch::VerifyMatchTime(LPTSTR pszMessage, WORD wMaxCount, WORD & wMessageType, bool bInGamePhase)
{
	//变量定义
	CTime CurTime = CTime::GetCurrentTime();
	CTime SignupStartTime(m_pMatchOption->MatchType.Immediate.SignupStartTime);
	CTime SignupEndTime(m_pMatchOption->MatchType.Immediate.SignupEndTime);
	CTime MatchStartTime(m_pMatchOption->MatchType.Immediate.MatchStartTime);
	...
	if (!bInGamePhase)
	{
		...
		if (CurTime < SignupStartTime)
		{
			...
			_sntprintf(pszMessage, wMaxCount, TEXT("报名尚未开始,请您于%d年%d月%d日-%d时%d分%d秒前来报名！"), SignupStartTime.GetYear(), SignupStartTime.GetMonth(), SignupStartTime.GetDay(), SignupStartTime.GetHour(), SignupStartTime.GetMinute(), SignupStartTime.GetSecond());
			return false;
		}
		...
		if (CurTime >= SignupEndTime)
		{
			...
			_sntprintf(pszMessage, wMaxCount, TEXT("报名已经结束,请您于%d年%d月%d日-%d时%d分%d秒再来报名！"), SignupStartTime.GetYear(), SignupStartTime.GetMonth(), SignupStartTime.GetDay()+7, SignupStartTime.GetHour(), SignupStartTime.GetMinute(), SignupStartTime.GetSecond());
			return false;
		}
	}
	else
	{
		...
		{
			wMessageType |= SMT_CLOSE_ROOM;
			_sntprintf(pszMessage, wMaxCount, TEXT("比赛已经开始,请您下次提早来参加比赛！"));
			return false;
		}
		...
		//{
		//	wMessageType |= SMT_CLOSE_ROOM;
		//	_sntprintf(pszMessage, wMaxCount, TEXT("比赛尚未开始,请您于%d年%d月%d日-%d时%d分%d秒来参加比赛！"), MatchStartTime.GetYear(), MatchStartTime.GetMonth(), MatchStartTime.GetDay(), MatchStartTime.GetHour(), MatchStartTime.GetMinute(), MatchStartTime.GetSecond());
		//	return false;
		//}
	}
...
bool CImmediateMatch::SendGameMessage(IServerUserItem * pIServerUserItem, LPCTSTR lpszMessage, WORD wMessageType)
{
	...
	if ((pIServerUserItem->GetBindIndex()!=INVALID_WORD)&&(pIServerUserItem->IsClientReady()==true)&&(pIServerUserItem->IsAndroidUser()==false))
	{
		...
		//发送数据
		DWORD wSendSize=sizeof(Message)-sizeof(Message.szString)+Message.wLength*sizeof(TCHAR);
...
//发送数据
bool CImmediateMatch::SendData(IServerUserItem * pIServerUserItem, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)
{
	if(pIServerUserItem!=NULL)
