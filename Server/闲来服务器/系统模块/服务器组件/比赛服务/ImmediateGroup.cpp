...
CImmediateGroup::CImmediateGroup(DWORD dwMatchNO,tagGameMatchOption * pGameMatchOption,IImmediateGroupSink *pIImmediateGroupSink)
{
	...
	//状态变量
	m_dwMatchNO = pGameMatchOption->dwMatchNO;//mChen hack //dwMatchNO;
...
bool CImmediateGroup::OnUserSignUpMatch(IServerUserItem * pUserItem)
{
	...
	//mChen add
	///m_enMatchStatus = MatchStatus_Wait_ForTime;

	//报名人满
	//mChen:待做：开个定时器，MatchStartTime时间到就检测是否人满开始
...
void CImmediateGroup::FirstRoundRuleInningsGameEnd(ITableFrame *pITableFrame)
{
	...
	//该桌打完指定局数
	if (ptagTableFrameInfo->cbPlayCount>=m_pMatchOption->MatchType.Immediate.cbPlayCount)//mChen
	{
	...
	{
		//自动换桌的控制
		if(true)//if(m_pMatchOption->MatchType.Immediate.cbSwitchTableCount>0&&(ptagTableFrameInfo->cbPlayCount%m_pMatchOption->MatchType.Immediate.cbSwitchTableCount)==0)//mChen edit
		{
