...
bool CAttemperEngineSink::OnEventTCPNetworkShut(DWORD dwClientAddr, DWORD dwActiveTime, DWORD dwSocketID)
{
	...
	//大厅服务
	if (pBindParameter->ServiceKind == ServiceKind_Lobby)
	{
		//变量定义
		WORD wBindIndex = LOWORD(dwSocketID);
		tagBindParameter * pBindParameter = (m_pBindParameter + wBindIndex);
		...
		//mChen add. for HideSeek
		//变量定义
		CMD_CS_S_LobbyRemove LobbyRemove;
		ZeroMemory(&LobbyRemove, sizeof(LobbyRemove));
		//删除通知
		LobbyRemove.wLobbyID = pBindParameter->wServiceID;
		m_pITCPNetworkEngine->SendDataBatch(MDM_CS_SERVICE_INFO, SUB_CS_S_LOBBY_REMOVE, &LobbyRemove, sizeof(LobbyRemove), 0L);

		//注销大厅
		m_GlobalInfoManager.DeleteLobbyItem(pBindParameter->wServiceID);
	}
...
bool CAttemperEngineSink::OnTCPNetworkMainRegister(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	{
		//mChen add, for HideSeek
	case SUB_CS_C_REGISTER_LOBBY:	//注册大厅
		{
			//效验数据
			ASSERT(wDataSize == sizeof(CMD_CS_C_RegisterLobby));
			if (wDataSize != sizeof(CMD_CS_C_RegisterLobby)) return false;

			//消息定义
			CMD_CS_C_RegisterLobby * pRegisterLobby = (CMD_CS_C_RegisterLobby *)pData;

			//有效判断
			if ((pRegisterLobby->szServerName[0] == 0) || (pRegisterLobby->szServerAddr[0] == 0))
			{
				//变量定义
				...
				//发送消息
				DWORD wStringSize = CountStringBuffer(RegisterFailure.szDescribeString);
				DWORD wSendSize = sizeof(RegisterFailure) - sizeof(RegisterFailure.szDescribeString) + wStringSize;
			...
			(m_pBindParameter + wBindIndex)->ServiceKind = ServiceKind_Lobby;

			//变量定义
			tagGameLobby GameLobby;
			ZeroMemory(&GameLobby, sizeof(GameLobby));

			//构造数据
			GameLobby.wLobbyID = wBindIndex;
			lstrcpyn(GameLobby.szServerName, pRegisterLobby->szServerName, CountArray(GameLobby.szServerName));
			lstrcpyn(GameLobby.szServerAddr, pRegisterLobby->szServerAddr, CountArray(GameLobby.szServerAddr));

			GameLobby.wLobbyPort = pRegisterLobby->wLobbyPort;

			//注册大厅
			m_GlobalInfoManager.ActiveLobbyItem(wBindIndex, GameLobby);

			//群发大厅
			m_pITCPNetworkEngine->SendDataBatch(MDM_CS_SERVICE_INFO, SUB_CS_S_LOBBY_INSERT, &GameLobby, sizeof(GameLobby), 0L);
...
bool CAttemperEngineSink::OnTCPNetworkMainManagerService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_CS_C_PROPERTY_TRUMPET:  //喇叭消息
		...
		//mChen add,for HideSeek
	case SUB_GP_LOBBY_CHAT:  //大厅聊天
		{
			//发送通知
			m_pITCPNetworkEngine->SendDataBatch(MDM_CS_MANAGER_SERVICE, SUB_GP_LOBBY_CHAT, pData, wDataSize, 0L);
			return true;
		}
	case SUB_CS_C_LOBBY_MESSAGE:  //大厅消息
		{
			//发送通知
			m_pITCPNetworkEngine->SendDataBatch(MDM_CS_MANAGER_SERVICE, SUB_CS_L_LOBBY_MESSAGE, pData, wDataSize, 0L);
			return true;
		}
	}
...
//房间发送
bool CAttemperEngineSink::SendDataToGame(WORD wServerID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)
{
	//查找房间
	CGlobalServerItem * pGlobalServerItem=m_GlobalInfoManager.SearchServerItem(wServerID);
...
//用户发送
bool CAttemperEngineSink::SendDataToUser(WORD wServerID, DWORD dwUserID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)
{
	return true;
}
