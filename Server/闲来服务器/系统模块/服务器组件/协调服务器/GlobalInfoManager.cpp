...
//构造函数
CGlobalLobbyItem::CGlobalLobbyItem()
{
	//设置变量
	m_wIndex = 0;
	ZeroMemory(&m_GameLobby, sizeof(m_GameLobby));

	//链表属性
	m_pNextLobbyItemPtr = NULL;
...
CGlobalInfoManager::CGlobalInfoManager()
{
	...
	//mChen add, for HideSeek
	m_pGlobalLobbyItem = NULL;
	m_MapLobbyID.InitHashTable(PRIME_SERVER);
...
CGlobalInfoManager::~CGlobalInfoManager()
{
	...
	//删除大厅
	WORD wLobbyID = 0;
	CGlobalLobbyItem * pGlobalLobbyItem = NULL;
	Position = m_MapLobbyID.GetStartPosition();
	while (Position != NULL)
	{
		m_MapLobbyID.GetNextAssoc(Position, wPlazaID, pGlobalLobbyItem);
		SafeDelete(pGlobalLobbyItem);
	}
	m_MapLobbyID.RemoveAll();
	...
	//mChen add, for HideSeek
	//删除大厅
	pGlobalLobbyItem = NULL;
	while (m_pGlobalLobbyItem != NULL)
	{
		pGlobalLobbyItem = m_pGlobalLobbyItem;
		m_pGlobalLobbyItem = m_pGlobalLobbyItem->m_pNextLobbyItemPtr;
		SafeDelete(pGlobalLobbyItem);
	}
...
//mChen add, for HideSeek
//删除大厅
bool CGlobalInfoManager::DeleteLobbyItem(WORD wLobbyID)
{
	//寻找广场
	CGlobalLobbyItem * pGlobalLobbyItem = NULL;
	if (m_MapLobbyID.Lookup(wLobbyID, pGlobalLobbyItem) == FALSE)
	{
		ASSERT(FALSE);
		return false;
	}
	...
	//释放大厅
	m_MapLobbyID.RemoveKey(wLobbyID);
	FreeGlobalLobbyItem(pGlobalLobbyItem);

	return true;
}
//激活大厅
bool CGlobalInfoManager::ActiveLobbyItem(WORD wBindIndex, tagGameLobby & GameLobby)
{
	//寻找大厅
	CGlobalLobbyItem * pGlobalLobbyItem = NULL;
	if (m_MapLobbyID.Lookup(GameLobby.wLobbyID, pGlobalLobbyItem) == TRUE)
	{
		ASSERT(FALSE);
		return false;
	}
	...
	//创建大厅
	pGlobalLobbyItem = CreateGlobalLobbyItem();
	if (pGlobalLobbyItem == NULL)
	{
		ASSERT(FALSE);
		return false;
	}
	...
	//设置大厅
	pGlobalLobbyItem->m_wIndex = wBindIndex;
	pGlobalLobbyItem->m_GameLobby = GameLobby;

	//设置索引
	m_MapLobbyID[GameLobby.wLobbyID] = pGlobalLobbyItem;
...
//mChen add, for HideSeek
//创建大厅
CGlobalLobbyItem * CGlobalInfoManager::CreateGlobalLobbyItem()
{
	//使用存储
	if (m_pGlobalLobbyItem != NULL)
	{
		CGlobalLobbyItem * pGlobalLobbyItem = m_pGlobalLobbyItem;
		m_pGlobalLobbyItem = m_pGlobalLobbyItem->m_pNextLobbyItemPtr;
		pGlobalLobbyItem->m_pNextLobbyItemPtr = NULL;
		return pGlobalLobbyItem;
	}

	//创建对象
	try
	{
		CGlobalLobbyItem * pGlobalLobbyItem = new CGlobalLobbyItem;
		return pGlobalLobbyItem;
	}
	catch (...) {}
...
//mChen add, for HideSeek
//释放大厅
bool CGlobalInfoManager::FreeGlobalLobbyItem(CGlobalLobbyItem * pGlobalLobbyItem)
{
	//效验参数
	ASSERT(pGlobalLobbyItem != NULL);
	if (pGlobalLobbyItem == NULL) return false;

	//设置变量
	pGlobalLobbyItem->m_wIndex = 0;
	ZeroMemory(&pGlobalLobbyItem->m_GameLobby, sizeof(pGlobalLobbyItem->m_GameLobby));

	//加入存储
	pGlobalLobbyItem->m_pNextLobbyItemPtr = m_pGlobalLobbyItem;
	m_pGlobalLobbyItem = pGlobalLobbyItem;
