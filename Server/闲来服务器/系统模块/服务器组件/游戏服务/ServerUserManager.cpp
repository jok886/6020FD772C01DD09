...
bool CServerUserItem::SetPlayerTeamType(PlayerTeamType eTeamType)
{
	//效验状态
	ASSERT(m_UserInfo.dwUserID != 0L);
	if (m_UserInfo.dwUserID == 0L) return false;

	m_UserInfo.cbTeamType = (BYTE)eTeamType;
	return true;
}
bool CServerUserItem::SetModelIndex(BYTE cbModelIndex)
{
	//效验状态
	ASSERT(m_UserInfo.dwUserID != 0L);
	if (m_UserInfo.dwUserID == 0L) return false;

	m_UserInfo.cbModelIndex = cbModelIndex;
	return true;
}
bool CServerUserItem::SetChoosedModelIndexOfTagger(BYTE cbChoosedModelIndexOfTagger)
{
	//效验状态
	ASSERT(m_UserInfo.dwUserID != 0L);
	if (m_UserInfo.dwUserID == 0L) return false;

	m_UserInfo.cbChoosedModelIndexOfTagger = cbChoosedModelIndexOfTagger;
	return true;
}
...
bool CServerUserItem::SetUserStatus(BYTE cbUserStatus, WORD wTableID, WORD wChairID)
{
	...
	m_UserInfo.cbLastUserStatus=m_UserInfo.cbUserStatus;
...
bool CServerUserItem::WriteUserScore(SCORE lScore, SCORE lGrade, SCORE lRevenue, DWORD dwUserMedal, BYTE cbScoreType, DWORD dwPlayTimeCount)
{
	...
	m_UserInfo.dwExperience += 10;// (dwPlayTimeCount + 59L) / 60L;
	...
	m_RecordInfo.dwExperience += 10;// (dwPlayTimeCount + 59L) / 60L;

	//变更信息
	m_VariationInfo.dwUserMedal+=dwUserMedal;
	m_VariationInfo.dwPlayTimeCount+=dwPlayTimeCount;
	m_VariationInfo.dwExperience += 10;// (dwPlayTimeCount + 59L) / 60L;
