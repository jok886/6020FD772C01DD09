...
struct tagGameMatchOption
{
	...
	union unGameMatchType
	{
		...
		struct tagLockTimeMatch
		{
			...
			//mChen add
			SYSTEMTIME						SignupStartTime;			//报名开始日期
			SYSTEMTIME						SignupEndTime;				//报名结束日期
		...
		struct tagImmediateMatch
		{
			...
			SYSTEMTIME						SignupStartTime;			//报名开始日期
			SYSTEMTIME						SignupEndTime;				//报名结束日期	
			SYSTEMTIME						MatchStartTime;				//游戏开始日期
...
public:
	...
	virtual BYTE GetLastUserStatus() = NULL;
...
public:
	...
	virtual bool SetPlayerTeamType(PlayerTeamType eTeamType) = NULL;
	virtual bool SetModelIndex(BYTE cbModelIndex) = NULL;
	virtual bool SetChoosedModelIndexOfTagger(BYTE cbChoosedModelIndexOfTagger) = NULL;
...
public:
	...
	virtual WORD GetPlayingUserCount() = NULL;
...
public:
	...
	//结束游戏
	virtual bool ConcludeGame(BYTE cbGameStatus, BYTE cbEndReason)=NULL;
...
public:
	...
	//发送数据
	virtual bool SendTableData(WORD wChairID, WORD wSubCmdID, VOID * pData, DWORD wDataSize,WORD wMainCmdID=MDM_GF_GAME)=NULL;
	//发送数据
	virtual bool SendLookonData(WORD wChairID, WORD wSubCmdID)=NULL;
	//发送数据
	virtual bool SendLookonData(WORD wChairID, WORD wSubCmdID, VOID * pData, DWORD wDataSize,WORD wMainCmdID=MDM_GF_GAME)=NULL;//,WORD wMainCmdID = MDM_GF_GAME
	//发送数据
	virtual bool SendUserItemData(IServerUserItem * pIServerUserItem, WORD wSubCmdID)=NULL;
	//发送数据
	virtual bool SendUserItemData(IServerUserItem * pIServerUserItem, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
...
public:
	...
	//设置私人场信息
	virtual void SetPrivateInfo(BYTE bGameTypeIdex,DWORD bGameRuleIdex,SCORE lBaseScore, BYTE PlayCout, BYTE PlayerCount)=NULL;
	//游戏重置 ZY add
	virtual void ResetPlayerTotalScore()=NULL;

	//mChen add, for HideSeek
	virtual void StartWaitGame() = NULL;
	virtual void HideSeek_FreeGameAndTimers(BYTE cbEndReason) = NULL;
	virtual void SendUsersInfoPacket(IServerUserItem * pIServerUserItem) = NULL;
	virtual WORD GetSitUserCountOfTeam(PlayerTeamType teamType) = NULL;
	virtual BYTE GetMapIndexRand() = NULL;
	virtual void SetMapIndexRand(BYTE cbMapIndexRand) = NULL;
	virtual BYTE GetChoosedMapIndex() = NULL;
	virtual void SetChoosedMapIndex(BYTE cbChoosedMapIndex) = NULL;
	virtual WORD GetRandSeed() = NULL;
	virtual WORD GetRandSeedForRandomGameObjec() = NULL;
	virtual WORD GetRandSeedForInventory() = NULL;
	virtual BYTE GetTheHumanHP(WORD wChairID) = NULL;
	virtual InventoryItem* GetInventoryList() = NULL;
...
public:
	...
	//mChen add, for HideSeek
	virtual void HideSeek_SendHeartBeat() = NULL;
	virtual void GenerateAICreateInfo() = NULL;
	virtual void HideSeek_SendAICreateInfo(IServerUserItem *pIServerUserItem=NULL, bool bOnlySendToLookonUser=false) = NULL;
	virtual WORD GetDeadHumanNumOfTeam(PlayerTeamType teamType) = NULL;
	virtual WORD GetDeadAINumOfTeam(PlayerTeamType teamType) = NULL;
	virtual bool AreAllPlayersOfTeamDead(PlayerTeamType teamType) = NULL;
	virtual BYTE GetHumanHP(WORD wChairID) = NULL;
	//道具同步
	virtual InventoryItem* GetInventoryList() = NULL;
	virtual void ResetInventoryList() = NULL;
	//使用复活道具修改玩家生命
	virtual void SetResurrection(WORD wChairID) = NULL;
	virtual void SetStealth(DWORD dwTime, WORD wChairID) = NULL;
	virtual void StealthUpate() = NULL;

	//网络接口
public:
	//游戏消息
	virtual bool OnGameMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem/*, IMainServiceFrame * pIMainServiceFrame*/)=NULL;
	//框架消息
...
{
	//用户断线
	virtual bool OnActionUserOffLine(WORD wChairID, IServerUserItem * pIServerUserItem, char *pFunc = NULL, char *pFile = __FILE__, int nLine= __LINE__) = NULL;
	//用户坐下
...
public:
	//发送数据
	virtual bool SendData(BYTE cbSendMask, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
	//发送数据
	virtual bool SendData(DWORD dwContextID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
	//发送数据
	virtual bool SendData(IServerUserItem * pIServerUserItem, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
	//群发数据
	virtual bool SendDataBatchToMobileUser(WORD wCmdTable, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
...
public:
	...
	//发送函数
	virtual bool SendSocketData(WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
...
public:
	//发送数据
	virtual bool SendDataToClient(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
	//发送数据
	virtual bool SendDataToClient(DWORD dwAndroidID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
	//发送数据
	virtual bool SendDataToServer(DWORD dwAndroidID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, DWORD wDataSize)=NULL;
};
