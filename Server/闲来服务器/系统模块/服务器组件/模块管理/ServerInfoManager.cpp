...
bool CServerInfoManager::ReadGameMatchOption(CDataBaseAide & PlatformDBAide, tagGameMatchOption & GameMatchResult)
{
	...
	if(GameMatchResult.cbMatchType==MATCH_TYPE_IMMEDIATE)
	{
		...
		//mChen add
		//时间配置
		PlatformDBAide.GetValue_SystemTime(TEXT("SignupStartTime"), GameMatchResult.MatchType.Immediate.SignupStartTime);
		PlatformDBAide.GetValue_SystemTime(TEXT("SignupEndTime"), GameMatchResult.MatchType.Immediate.SignupEndTime);
		PlatformDBAide.GetValue_SystemTime(TEXT("MatchStartTime"), GameMatchResult.MatchType.Immediate.MatchStartTime);
	}
