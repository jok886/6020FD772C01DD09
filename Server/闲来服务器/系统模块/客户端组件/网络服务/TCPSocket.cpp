...
//发送函数
WORD  CTCPSocket::SendData(WORD wMainCmdID, WORD wSubCmdID, void * pData, DWORD wDataSize)
{
	//效验状态
	if (m_hSocket == INVALID_SOCKET) return false;
//	if (m_cbSocketStatus != SOCKET_STATUS_CONNECT) return false;
...
//发送数据
DWORD CTCPSocket::SendDataBuffer(void * pBuffer, DWORD wSendSize)
{
	//效验参数
	ASSERT(wSendSize!=0);
	ASSERT(pBuffer!=NULL);
...
//加密数据
DWORD CTCPSocket::EncryptBuffer(BYTE pcbDataBuffer[], DWORD wDataSize, DWORD wBufferSize)
{
	int i = 0;
	//效验参数
...
//解密数据
DWORD CTCPSocket::CrevasseBuffer(BYTE pcbDataBuffer[], DWORD wDataSize)
{
	//效验参数
