...
BEGIN
	...
	SELECT @MedalRate=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'MedalRate'

	-- 银行税率
	DECLARE @RevenueRate AS INT
	SELECT @RevenueRate=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRate'

	-- 兑换比率
	DECLARE @ExchangeRate AS INT
	SELECT @ExchangeRate=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'MedalExchangeRate'

	-- 赢局经验
	DECLARE @WinExperience AS INT
	SELECT @WinExperience=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'WinExperience'	
	...
	FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.GameGameItem TableGame,QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.GameKindItem TableKind
