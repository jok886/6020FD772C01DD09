...
BEGIN	
	...
	SELECT @IsAndroidUser=IsAndroid,@UserMemberOrder=MemberOrder FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	BEGIN
		...
		BEGIN	
			...
			BEGIN
				...
				-- 更新金币
				UPDATE QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo SET Score=@Score-@lMatchFee WHERE UserID=@dwUserID				
			END
			...
			BEGIN
				...
				-- 更新元宝
				UPDATE QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo SET UserMedal=@wUserMedal-@lMatchFee WHERE UserID=@dwUserID	
			END
		END
	END
	...
	SELECT @CurrScore=Score FROM QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo WHERE UserID=@dwUserID		
	
	-- 查询元宝
	DECLARE @CurrIngot BIGINT	
	SELECT @CurrIngot=UserMedal FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE UserID=@dwUserID
...
BEGIN
	...
	-- 用户信息
	DECLARE @IsAndroidUser TINYINT	
	SELECT @IsAndroidUser=IsAndroid FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	BEGIN
		...
		BEGIN
			...
			-- 更新元宝
			UPDATE QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo SET UserMedal=@UserMedal+@lMatchFee WHERE UserID=@dwUserID	
		END	
	END
