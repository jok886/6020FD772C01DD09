...
BEGIN
	...
	-- 系统暂停
	SELECT @EnjoinInsure=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'EnjoinInsure'
	IF @EnjoinInsure IS NOT NULL AND @EnjoinInsure<>0
	BEGIN
		SELECT @strErrorDescribe=StatusString FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'EnjoinInsure'
		RETURN 2
	END
	...
	SELECT @BankPrerequisite=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'BankPrerequisite'
	...
	SELECT @MatchLockKindID=KindID, @MatchLockServerID=ServerID FROM QPGameMatchDB_HideSeek..GameScoreLocker WHERE UserID=@dwUserID
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @MatchKindName=KindName FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.GameKindItem WHERE KindID=@MatchLockKindID
			SELECT @MatchServerName=ServerName FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.GameRoomInfo WHERE ServerID=@MatchLockServerID

			-- 错误信息
			IF @MatchKindName IS NULL SET @KindName=N'未知游戏'
			IF @MatchServerName IS NULL SET @ServerName=N'未知房间'
			SET @strErrorDescribe=N'您正在 [ '+@MatchKindName+N' ] 的 [ '+@MatchServerName+N' ] 比赛房间中，不能进行当前的保险柜操作！'
			RETURN 4
		END
		ELSE
		BEGIN
...
BEGIN
	...
	SELECT @RevenueRate=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTake'
...
BEGIN
	...
	SELECT @TransferPrerequisite=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'TransferPrerequisite'
	...
	IF @cbByNickName=1
		SELECT @TargetUserID=UserID FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE NickName=@strNickName
	ELSE
		SELECT @TargetUserID=UserID FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE GameID=@strNickName
	...
	SELECT @RevenueRate=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTransfer'
	SELECT @MaxTax=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'TransferMaxTax'
	...
	SELECT @TransferRetention=ISNULL(StatusValue,0) FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'TransferRetention'
...
BEGIN
	...
	DECLARE @TransferPrerequisite AS BIGINT
	SELECT @RevenueRateTake=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTake'
	SELECT @RevenueRateTransfer=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTransfer'
	SELECT @TransferPrerequisite=StatusValue FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'TransferPrerequisite'
