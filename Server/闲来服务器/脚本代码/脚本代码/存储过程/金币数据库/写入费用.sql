...
BEGIN	
	...
	SET @ReturnValue=0
	SELECT @UserRight=UserRight FROM QPAccountsDB_HideSeek..AccountsInfo WHERE UserID=@dwUserID 
	SELECT @UserRight=@UserRight|UserRight FROM GameScoreInfo WHERE UserID=@dwUserID

	IF EXISTS(SELECT * FROM QPTreasureDB_HideSeek..GameScoreInfo WHERE UserID=@dwUserID)
	BEGIN
		DECLARE @Score BIGINT
		SELECT @Score=Score FROM QPTreasureDB_HideSeek..GameScoreInfo WHERE UserID=@dwUserID
		IF @Score < @dwMatchFee
		BEGIN
			SET @ReturnValue=1
		END
		ELSE
		BEGIN
			UPDATE QPTreasureDB_HideSeek..GameScoreInfo SET Score=Score-@dwMatchFee WHERE UserID=@dwUserID
