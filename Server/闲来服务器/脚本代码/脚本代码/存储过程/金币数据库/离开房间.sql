...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GR_JoinFalied]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GR_JoinFalied]
GO

IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GR_ClearGameScoreLocker]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GR_ClearGameScoreLocker]
GO
...
BEGIN
	...
	BEGIN
		...
		@CellOnlinePresent=CellOnlinePresent,@CellOnlineTime=CellOnlineTime,@StartOnlineTime=StartOnlineTime,
		@IsPlayPresent=IsPlayPresent,@IsOnlinePresent=IsOnlinePresent
		FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.GlobalPlayPresent WHERE ServerID=-1
		IF @ServerID IS NULL
		BEGIN
			SET @IsPlayPresent=0
...
-- 清除锁表
CREATE PROC GSP_GR_ClearGameScoreLocker
