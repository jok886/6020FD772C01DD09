...
BEGIN
	...
	IF @iRankIdex = 0
	BEGIN
		SELECT * FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.WeakAddRankConfig	
	END
	
	IF @iRankIdex = 1
	BEGIN
		SELECT * FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.QuarterAddRankConfig	
	END
	
	IF @iRankIdex = 2
	BEGIN
		SELECT * FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.YearAddRankConfig	
	END
...
BEGIN
	...
	BEGIN
		...
		DECLARE C_Employees CURSOR FAST_FORWARD FOR
			...
			FROM QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo
			ORDER BY Score DESC
...
BEGIN
	...
	SELECT @LastWeakRank=DayValue FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.AddRankConfig WHERE RankName=N'LastWeakRank'
	...
	SELECT @LastQuarterRank=DayValue FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.AddRankConfig WHERE RankName=N'LastQuarterRank'
	...
	SELECT @LastYearWeakRank=DayValue FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.AddRankConfig WHERE RankName=N'LastYearWeakRank'
	...
	BEGIN
		...
		BEGIN--周奖励
			...
			BEGIN
				...
				SELECT @RewardGold=RewardGold,@RewardType=RewardType FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.WeakAddRankConfig WHERE DayID=@DayIdex
			...
			--------------------------------------------
			--
			UPDATE QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.AddRankConfig SET DayValue=getdate()	
	...
	BEGIN
		...
		BEGIN --季度奖励
			...
			BEGIN
				...
				SELECT @RewardGold=RewardGold,@RewardType=RewardType FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.QuarterAddRankConfig WHERE DayID=@DayIdex
				IF @RewardGold <> NULL and @RewardType <> NULL
				BEGIN
					IF @RewardType = 1
					BEGIN
	...
	BEGIN
		...
		BEGIN --年奖励
			...
			BEGIN
				...
				SELECT @RewardGold=RewardGold,@RewardType=RewardType FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.YearAddRankConfig WHERE DayID=@DayIdex
				IF @RewardGold <> NULL and @RewardType <> NULL
				BEGIN
					IF @RewardType = 1
					BEGIN
