...
BEGIN
	...
	SELECT @Nullity=Nullity, @CurrMemberOrder=MemberOrder FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE UserID=@dwSourceUserID
	...
	-- 目标用户
	IF @dwTargetUserID<>0 AND NOT EXISTS( SELECT UserID FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE UserID=@dwTargetUserID )
	BEGIN
	...
	BEGIN
		...
		SELECT @Insure=InsureScore FROM QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo WHERE UserID=@dwSourceUserID
		...
		-- 银行扣费
		UPDATE QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo SET InsureScore=InsureScore-@ConsumeGold WHERE UserID=@dwSourceUserID	
	END
	ELSE
	BEGIN
	...
	BEGIN
		...
		-- 删除过期
		DELETE FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsMember
		WHERE UserID=@dwTargetUserID AND MemberOrder=@MemberOrder AND MemberOverDate<=GETDATE()

		-- 更新会员
		UPDATE QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsMember SET MemberOverDate=DATEADD(mm,@MemberValidMonth, GETDATE())
		WHERE UserID=@dwTargetUserID AND MemberOrder=@MemberOrder
		IF @@ROWCOUNT=0
		BEGIN
			INSERT QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsMember(UserID,MemberOrder,UserRight,MemberOverDate)
		...
		FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsMember WHERE UserID=@dwTargetUserID

		-- 会员权限
		SELECT @MaxUserRight=UserRight FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsMember
		WHERE UserID=@dwTargetUserID AND MemberOrder=@MaxMemberOrder
		
		-- 附加会员卡信息
		UPDATE QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo
	...
	-- 会员等级
	SELECT @CurrMemberOrder=MemberOrder FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE UserID=@dwTargetUserID

	-- 购买记录
	IF @wPropertyID<=13 -- 礼物
	BEGIN
		INSERT QPRecordDB_HideSeekLink.QPRecordDB_HideSeek.dbo.RecordSendPresent(PresentID,RcvUserID,SendUserID,LovelinessRcv,LovelinessSend,PresentPrice,PresentCount,KindID,ServerID,SendTime,ClientIP)
	...
	ELSE
	BEGIN -- 道具
		INSERT QPRecordDB_HideSeekLink.QPRecordDB_HideSeek.dbo.RecordSendProperty(PropID,SourceUserID,TargetUserID,PropPrice,PropCount,KindID,ServerID,SendTime,ClientIP)
