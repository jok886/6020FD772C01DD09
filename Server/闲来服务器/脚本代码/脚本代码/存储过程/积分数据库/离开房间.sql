...
BEGIN
	...
	FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.GlobalPlayPresent WHERE ServerID=@wServerID
	IF @ServerID IS NULL
	BEGIN
		SELECT @ServerID=ServerID,@PresentMember=PresentMember,@CellPlayPresnet=CellPlayPresnet,@CellPlayTime=CellPlayTime,@StartPlayTime=StartPlayTime,
		@CellOnlinePresent=CellOnlinePresent,@CellOnlineTime=CellOnlineTime,@StartOnlineTime=StartOnlineTime,
		@IsPlayPresent=IsPlayPresent,@IsOnlinePresent=IsOnlinePresent
		FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.GlobalPlayPresent WHERE ServerID=-2
		IF @ServerID IS NULL
		BEGIN
			SET @IsPlayPresent=0
	...
	FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.GlobalPlayPresent WHERE ServerID=-3
	...
	BEGIN
		...
		SELECT @UserDatePresent=PresentScore FROM QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.StreamPlayPresent WHERE DateID=@DateID AND UserID=@dwUserID
	...
	BEGIN
		...
		SELECT @UserPresent=SUM(PresentScore) FROM QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.StreamPlayPresent WHERE UserID=@dwUserID
    ...
    SELECT @MemberOrder=MemberOrder FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	BEGIN		
		-- 更新用户金币
		UPDATE QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo set Score=Score+@PresentScore WHERE UserID=@dwUserID

		UPDATE QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.StreamPlayPresent SET PresentCount=PresentCount+1,PresentScore=PresentScore+@PresentScore,
		...
		IF @@ROWCOUNT=0
		BEGIN
			INSERT QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.StreamPlayPresent (DateID,UserID,PresentCount,PresentScore,PlayTimeCount,OnLineTimeCount)
