...
BEGIN
	...
	-- 查询奖励
	SELECT * FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.BeginnerConfig	
...
BEGIN
	...
	SELECT @lRewardGold=RewardGold,@lRewardType=RewardType FROM QPPlatformDB_HideSeekLink.QPPlatformDB_HideSeek.dbo.BeginnerConfig WHERE DayID=(@wSeriesDate+1)
	...
	BEGIN
		-- 奖励金币	
		UPDATE QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo SET Score = Score + @lRewardGold WHERE UserID = @dwUserID
	...
	SELECT @lScore=Score FROM QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo WHERE UserID = @dwUserID 	
