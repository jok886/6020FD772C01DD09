...
BEGIN
	...
	SELECT @Score=Score,@InsureScore=InsureScore FROM QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo WHERE UserID=@dwUserID
	IF @@rowcount = 0
	BEGIN
		-- 插入资料
	...
	-- 领取金币	
	UPDATE QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo SET Score = Score + @ScoreAmount WHERE UserID = @dwUserID

	-- 查询金币
	SELECT @Score=Score FROM QPTreasureDB_HideSeekLink.QPTreasureDB_HideSeek.dbo.GameScoreInfo WHERE UserID = @dwUserID 	

	-- 输出提示
	SET @strNotifyContent = N'获得东山再起基金 '+convert(varchar,@ScoreAmount)+' 金币！您还剩 '+CAST(@TakeTimes-@TodayTakeTimes AS NVARCHAR)+N' 次机会！'
