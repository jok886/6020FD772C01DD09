...
CREATE PROC GSP_GP_ModifyUserIndividual
	...
	@strClientIP NVARCHAR(15),					-- 连接地址
	@wModCost SMALLINT,                         -- 修改所需花费		
	@cbModCosType TINYINT,						-- 花费类型：0-金币，1-钻石
	@strErrorDescribe NVARCHAR(127) OUTPUT		-- 输出信息
WITH ENCRYPTION AS
...
BEGIN
	...
	BEGIN
		INSERT INTO QPRecordDB_HideSeekLink.QPRecordDB_HideSeek.dbo.RecordAccountsExpend(UserID,ReAccounts,ClientIP)
		...
		FROM QPTreasureDB_HideSeek.dbo.GameScoreInfo(NOLOCK) 
		WHERE UserID=@UserID
		...
		BEGIN
			...
			-- 扣除费用
			SET @Score = @Score-@wModCost
			UPDATE QPTreasureDB_HideSeek.dbo.GameScoreInfo SET Score=@Score WHERE UserID=@UserID
		END
		ELSE IF @cbModCosType=1
		BEGIN
			--花费钻石
			...
			-- 扣除费用
			SET @Insure = @Insure-@wModCost
			UPDATE QPTreasureDB_HideSeek.dbo.GameScoreInfo SET InsureScore=@Insure WHERE UserID=@UserID
		END ELSE 
		BEGIN
			SET @strErrorDescribe=N'花费类型不对！'
			RETURN 8
		END
	END
