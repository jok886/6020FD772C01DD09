...
BEGIN
	...
	FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE UserID=@dwSrcUserID
	...
	FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo WHERE UserID=@dwDstUserID
	...
	SELECT @TransferPrerequisite=ISNULL(StatusValue,0) FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.SystemStatusInfo WHERE StatusName=N'TransferPrerequisite'
	IF @dwSwapScore <= @TransferPrerequisite
	BEGIN
		SET @strErrorDescribe=N'非常抱歉,转账金币数必须大于'+LTRIM(@TransferPrerequisite)+'才可转账'
		RETURN 5
	END
