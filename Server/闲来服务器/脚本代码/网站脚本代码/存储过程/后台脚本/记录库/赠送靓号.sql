...
BEGIN
	...
	-- 获取用户游戏ID
	SELECT @CurGameID = GameID FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo
	WHERE UserID = @UserID

	-- 判断
	SELECT @dwGameID = GameID,@IDLevel = IDLevel FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.ReserveIdentifier
	...
	SELECT @dwUserID = UserID FROM QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo
	WHERE GameID = @ReGameID
	...
	-- 更新保留表
	UPDATE QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.ReserveIdentifier SET Distribute = 1 
	WHERE GameID = @ReGameID

	-- 更新用户表
	UPDATE QPAccountsDB_HideSeekLink.QPAccountsDB_HideSeek.dbo.AccountsInfo SET GameID = @ReGameID
	WHERE UserID = @UserID
